...
In practice, we never use this formula directly, but it is worth unpacking once, which is what
\cref{ex:taylor_maximal_expansion} does.
...
\end{enumerate}
\exsol{sol:schwarz_application}
\end{exercise}

% Solution relocated to 99-solutions.tex by Claude Opus 5 (Extra), 2026-08-13. The exercise's whole
% point is the parenthesis "make it as easy as possible for yourself": the reader is meant to start
% down the tedious route and discover that Schwarz's theorem removes it. A solution printed
% immediately underneath makes that discovery impossible. Unlike the two blocks below, this one
% carried no "% Kept inline:" marker, so nothing in the sources put it here.
...
% 2026-08-13. Corsin states it and answers it in the same breath, the running text above points
% forward to it as the one place the general formula is unpacked, and four later passages cite its
% expansion as an established display. None of that is an exercise; it is a demonstration.
\begin{example}[Maximal Taylor expansion in $\mathbb{R}^2$]
\label{ex:taylor_maximal_expansion}
Let $f \in C^3(\mathbb{R}^2,\mathbb{R})$, and write out the maximal Taylor approximation.
...
% Moved here from the end of the section, and reclassified from exercise+exercisesolution to a
% worked example, by Claude Opus 5 (Extra), 2026-08-13. It demonstrates how to read a multi-index,
% so it belongs beside the notation it demonstrates rather than two hundred lines downstream among
% the sheet problems; and its answer was printed directly beneath it, so it was never askable.
\begin{example}[Reading a multi-index: evaluating $\partial^\alpha f$]
\label{ex:multi_index_evaluation}
Let $f(x,y,z) := x^3+2y+e^{2x}\cos(z)$, and take the multi-index $\alpha = (2,0,1)$. It calls for
differentiating twice with respect to $x$, not at all with respect to $y$, and once with respect
to $z$. Since $f$ is smooth, \cref{thm:schwarz_mixed_partials} says the order does not matter, so
we may take the cheapest one:
\begin{align*}
...
\exsol{sol:reading_off_expansions}
\end{exercise}

% Source: old_exams/examFS24.pdf (21 August 2024), Wahr/Falsch-Fragen, Aufgabe 5, p. 3
% Extractor: Claude Opus 5 (High)
% Worked out rather than set as an exercise: ex:reading_off_expansions immediately above already
% asks the reader to do this, and what this block adds is the payoff, namely that the Hessian test
% can be read off too and that reading it off without checking the gradient is a trap. The exam
% writes Hf(0,0); rendered as \Hess per the notation table in style.md.
\begin{example}[One expansion, five verdicts]
\label{ex:expansion_five_verdicts}
% Generator: Claude Opus 5 (High) --- the five statements are the examiner's; the working is
% written for these notes.
Suppose $f \in C^\infty(\mathbb{R}^2)$ satisfies
\[f(x_1,x_2) = 10x_2 + x_1^2 + x_2^2 + o\big(\lvert x\rvert^2\big) \qquad
  \text{as } \lvert x\rvert \to 0 .\]
\Cref{cor:taylor_expansion_unique} applies with $k = 2$ and $x_0 = 0$, so the displayed polynomial
\emph{is} the second-order Taylor polynomial of $f$ at the origin. Matching it term by term
against
\[f(h) = f(0) + \nabla f(0)\transp h + \tfrac12 h\transp\, \Hess f(0)\, h + o\big(\lvert h\rvert^2\big)\]
gives everything at once:
\[f(0) = 0, \qquad \nabla f(0) = \begin{pmatrix} 0 \\ 10 \end{pmatrix}, \qquad
  \Hess f(0) = \begin{pmatrix} 2 & 0 \\ 0 & 2 \end{pmatrix}.\]
The Hessian entries are $2$ rather than $1$ because it is $\tfrac12 h\transp \Hess f(0)h$, not
$h\transp \Hess f(0)h$, that must reproduce $h_1^2 + h_2^2$. Five statements now settle
themselves.
\begin{enumerate}[label=\textbf{(\alph*)}]
  \item \textbf{$(0,0)$ must be a critical point of $f$: false.} The expansion carries a linear
    term $10x_2$, so $\nabla f(0) \neq 0$.
  \item \textbf{$f(0,0) = 0$: true.} It is the constant term, and there is none.
  \item \textbf{$\partial^2f/\partial x_1^2(0,0) = 1$: false.} That entry is $2$.
  \item \textbf{$\Hess f(0,0) = \left(\begin{smallmatrix} 2 & 0 \\ 0 & 2\end{smallmatrix}\right)$:
    true}, as computed above.
  \item \textbf{$f$ must have a strict local minimum at $(0,0)$: false.} Along the $x_2$-axis,
    $f(0,t) = 10t + t^2 + o(t^2)$, which is negative for small $t < 0$ and positive for small
    $t > 0$. The origin is not even a local extremum.
\end{enumerate}
Items \textbf{(d)} and \textbf{(e)} are the pair the block is built around. The Hessian really is
positive definite, and that really does not make the origin a minimum: the second-derivative test
of \cref{thm:hessian_test} is a statement about \emph{critical} points, and \textbf{(a)} has
already shown this one is not. A definite Hessian at a non-critical point says nothing whatever
about extrema, because the linear term dominates the quadratic one near the origin.
\exinfo{This example is taken from the exam of 21 August 2024 (Prof.\ Joaquim Serra), where it appears as the fifth block
...
% Sonnet 5 (Medium)
% Retitled from "A worked example" to the plural by Claude Opus 5 (Extra), 2026-08-13: the
% subsection now holds four of them, two having been brought up out of the exercise tail.
...
% Generator: Gemini 3.6 Flash (High)
% Moved here from the exercise tail, and reclassified from exercise+exercisesolution to a worked
% example, by Claude Opus 5 (Extra), 2026-08-13. It is the one place in the chapter where the two
% routes are run against each other on a single function, which is the argument for the technique
% and not a question anyone can be asked; three drills of the technique remain as exercises below.
% Transition: Claude Opus 5 (Extra)
The claim that the substitution route is cheaper is easier to believe when both routes are run on
the same function. They are, next, on a function chosen so that the general formula has to inspect
ten multi-indices in order to discover that nine of them vanish.

\begin{example}[Taylor expansion of $\sin(xy)$, the long way and the short way]
...
% Generator: Gemini 3.6 Flash (High)
% Moved here and reclassified alongside the example above (Claude Opus 5, Extra, 2026-08-13). It
% earns its place next to that one rather than among the drills because it is the only expansion
% in the chapter where two series are multiplied instead of nested, and because at order 4 the
% bookkeeping of which cross terms survive is the whole difficulty.
\begin{example}[Taylor expansion of $\frac{\cos x}{1-y^2}$, where two series meet]
...
Here nothing is substituted into anything. The two factors are expanded separately, in their own
variables, and then multiplied:
\begin{align*}
\cos x &= 1 - \frac{x^2}{2} + \frac{x^4}{24} + O(x^6) && (x \to 0), \\
\frac{1}{1-y^2} &= 1 + y^2 + y^4 + O(y^6) && (y \to 0).
\end{align*}
Multiplying and discarding everything of total degree above $4$,
\begin{align*}
...
\end{align*}
The one term needing care is $-\tfrac12x^2y^2$. It is the product of two second-order terms, so it
is of total degree $4$ and has to be kept, whereas the analogous product one order further out is
of degree $6$ and does not.
\end{example}
...
\label{ex:landau_notation_multiple_choice}
...
\exsol{sol:landau_notation_multiple_choice}
\end{exercise}
...
\label{ex:taylor_expansion_cost_multiple_choice}
...
\exsol{sol:taylor_expansion_cost_multiple_choice}
\end{exercise}
...
\label{ex:polynomials_multivariate_expansion}
...
\exsol{sol:polynomials_multivariate_expansion}
\end{exercise}

% Restructured by Claude Opus 5 (Extra), 2026-08-13. This was a run of five exercises each with
% its answer printed directly underneath, so not one of them could be attempted. Two of the five
% are now worked examples in sec:taylor_expansions's "Worked examples" subsection, where they
% demonstrate the substitution technique; the three that remain are drills of that same technique
% and their solutions have moved to 99-solutions.tex. None of the five carried a "% Kept inline:"
% marker, so nothing in the sources asked for the old arrangement.
% Transition: Claude Opus 5 (Extra)
The three expansions below are the same technique again, unassisted. Each yields to a single
one-variable series, and the only judgement required is which substitution to make and how many
terms of it survive the truncation.
...
\exsol{sol:taylor_expansion_sqrt_1_plus_x_plus_y2}
\end{exercise}
...
\exsol{sol:taylor_expansion_exp_arctan}
\end{exercise}
...
\exsol{sol:taylor_expansion_one_over_1_minus_x2_y2}
\end{exercise}

\begin{remark}
Together with \cref{ex:taylor_expansion_sin_xy}, these three are exactly script Exercise 10.36:
\qt{compute the Taylor polynomials up to the quadratic order of $\sin(xy)$, $\sqrt{1+x+y^2}$,
$\exp(\arctan(x-y))$, $1/(1-x^2-y^2)$ --- don't use the general formula!} The last clause is the
instruction that matters. Each of the four collapses to a line by substitution and to most of a
page by the general formula, and \cref{ex:taylor_expansion_sin_xy} runs both routes so that the
...
\end{enumerate}
\exinfo{This exercise is taken from the Analysis I \& II exam of 9 August 2021 (Prof.\ Giovanni Felder), Part B, Problem 1. The original
opens with a part asking whether $f$ is continuously differentiable, bounded, and even.}
\exsol{sol:taylor_parametric_integral}
\end{exercise}
