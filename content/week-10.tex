...
\section{Bounded $C^1$ domains and why the definition in the script is so terrible}
...
sub-part \textbf{5.} --- the harder \qt{iff} characterization of a bounded $C^1$ domain,
...
$$\Omega := \left\{(x,y,z)\in\mathbb{R}^3 \mid a<x<b,\ \sqrt{y^2+z^2}<f(x)\right\}, \qquad
\partial_{\mathrm{side}}\Omega := \left\{(x,y,z)\in\mathbb{R}^3 \mid a<x<b,\ \sqrt{y^2+z^2}
  =f(x)\right\}.$$
\begin{enumerate}[label=\textbf{\arabic*.}]
