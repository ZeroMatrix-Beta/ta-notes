...
\begin{figurestub}{FIG-W02-01}
Four concentric rounded blobs (irregular circle-like shapes, not perfect circles), each nested
fully inside the next, largest to smallest: outermost red blob labelled \textit{Topological
spaces} (label near the top, inside the red band); yellow blob inside it labelled \textit{Metric
spaces}; green blob inside that labelled \textit{Normed vector spaces}; innermost blue blob
labelled \textit{Inner product spaces}. Each label sits inside its own coloured band, stacked top
to bottom in the same nesting order.
\end{figurestub}
...
\begin{figurestub}{FIG-W02-02}
On a pair of coordinate axes (origin at bottom-left): point $x$ sits near the origin
(bottom-left), $y$ sits upper-middle, $z$ sits upper-right, so the three points form a flattened,
lever-like triangle. A blue two-segment path traces $x \to y \to z$ (up, then across). A red
straight edge connects $x$ directly to $z$, cutting beneath the blue path. Below the sketch, the
annotation $d(x,z) \le d(x,y) + d(y,z)$ in red.
\end{figurestub}
