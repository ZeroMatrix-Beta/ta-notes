...
\begin{center}
% Correction: Claude Opus 5 (High) --- the curve attained neither marked extremum. It was
% plot[smooth, tension=0.8] through (0.3,0.8) (0.9,2.3) (2.1,0.4) (3.0,1.8), and `smooth' makes
% the tangent at an interior point parallel to the chord of its NEIGHBOURS: at c the chord was
% (2.1,0.4)-(0.3,0.8), slope -0.22, so the curve was already descending at c and its apex lay to
% the LEFT of c, above f(c); at d the chord was (3.0,1.8)-(0.9,2.3), slope -0.24, so the trough
% lay to the RIGHT of d, below f(d). Both visible at 700 dpi. The caption asserts the maximum is
% at c and the minimum at d, so the figure contradicted its own caption -- and f(b)=1.8 was in
% any case below f(c)=2.3 only by luck of the interpolation.
%
% Now an explicit cubic, the FIG-W06-03 pattern, so the extrema are exactly where they are drawn:
%   f(x) = 2.1991x^3 - 9.8958x^2 + 12.4687x - 2.5094 = 6.5972*(x^3/3 - 1.5x^2 + 1.89x) - 2.5094,
%   f'(x) = 6.5972*(x-0.9)(x-2.1), zero exactly at c=0.9 and d=2.1, positive outside, negative
%   between -- so c is a strict local max and d a strict local min.
% Values: f(0.45)=1.298, f(0.9)=2.300, f(2.1)=0.400, f(2.6)=1.665. Over [a,b]=[0.45,2.6] that
% makes f(c)=2.3 the strict global max and f(d)=0.4 the strict global min, both attained in the
% interior, which is the point the prose below draws from the picture. b moved 3.0 -> 2.6
% because g(2.7)=g(0.9) exactly: at the old b the curve came back up TO the maximum value.
% a moved 0.3 -> 0.45 because g(0.3)=g(2.1) exactly: at the old a it came down to the minimum.
\begin{tikzpicture}[>=Latex, scale=1.2]
  \draw[->] (-0.3,0) -- (3.5,0) node[right] {$x$};
  \draw[->] (0,-0.3) -- (0,2.8) node[above] {$y$};

  \draw[green!60!black, thick, domain=0.45:2.6, samples=80, smooth]
    plot ({\x}, {2.1991*\x*\x*\x - 9.8958*\x*\x + 12.4687*\x - 2.5094});

  \draw (0.45,0.05) -- (0.45,-0.05) node[below] {$a$};
  \draw (0.9,0.05) -- (0.9,-0.05) node[below] {$c$};
  \draw (2.1,0.05) -- (2.1,-0.05) node[below] {$d$};
  \draw (2.6,0.05) -- (2.6,-0.05) node[below] {$b$};
...
\end{enumerate}
\exsol{sol:classification_table}
\end{exercise}
...
\label{ex:open_closed_complete_compact}
...
\exsol{sol:open_closed_complete_compact}
\end{exercise}
...
\label{ex:open_complete_compact_multiple_choice}
...
\exsol{sol:open_complete_compact_multiple_choice}
\end{exercise}
...
\end{enumerate}
\exinfo{This exercise is taken from the exam of 21 August 2024 (Prof.\ Joaquim Serra), where it appears as the first block
of true/false questions. It is reformulated here as a proof exercise.}
\exsol{sol:annulus_continuous_image}
\end{exercise}
...
\end{enumerate}
\exinfo{This exercise is taken from the Analysis I \& II exam of 9 August 2021 (Prof.\ Giovanni Felder), Part B, Problem 3.}
\exsol{sol:nearest_point_to_origin}
\end{exercise}

% Source: old_exams/FS20.pdf (Analysis I & II, 12 August 2020, Prof. Peter S. Jossen),
% Teil B, Aufgabe 2, parts (c)--(e), p. 19
% Extractor: Claude Opus 5 (High)
% The paper's parts (a) and (b) are omitted: (a) asks for the definition of an open cover, and (b)
% for a direct proof that (0,1] is not compact, which the (unlabelled) aiexample "Open Cover of
% (0,1) with No Finite Subcover" in 04-heine-borel.tex already does for (0,1).
\begin{exercise}[What boundedness of every continuous function buys]
\label{ex:pseudocompactness_implications}
Let $(X,d)$ be a non-empty metric space and consider the following four statements about it.
\begin{itemize}
  \item \textbf{(i)} Every continuous function $X \to \mathbb{R}$ is bounded.
  \item \textbf{(ii)} Every continuous function $X \to \mathbb{R}$ attains a maximum and a minimum.
  \item \textbf{(iii)} $X$ is compact.
  \item \textbf{(iv)} Every sequence in $X$ has an accumulation point.
\end{itemize}
\begin{enumerate}[label=\textbf{(\alph*)}]
  \item Prove the equivalence \textbf{(i)} $\iff$ \textbf{(ii)}.
  \item Prove the implication \textbf{(iii)} $\implies$ \textbf{(i)}, working directly from the
    open-cover definition of compactness.
  \item Prove the implication \textbf{(iv)} $\implies$ \textbf{(i)}.
\end{enumerate}
\exinfo{This exercise is taken from the Analysis I \& II examination of 12 August 2020
(Prof.\ Peter S.\ Jossen), Part B, Problem 2. Its first two parts, on the definition of an open
cover and on the non-compactness of $(0,1]$, are omitted here.}
\exsol{sol:pseudocompactness_implications}
\end{exercise}

% Source: old_exams/FS19.pdf (Analysis I & II, 15 August 2019, Prof. Peter S. Jossen),
% Teil B, Aufgabe 2, parts (d)--(f), p. 24
% Extractor: Claude Opus 5 (High)
% Parts (a)--(c) of the original ask for the definitions of open, closed and compact plus one
% example of each, which chapters 3 and 7 already carry.
\begin{exercise}[Sums of sets, and which property survives]
\label{ex:sumset_open_closed_compact}
For subsets $A, B \subseteq \mathbb{R}^2$ write $A + B := \{a+b \mid a \in A,\ b \in B\}$. Prove
the following, using in each case whichever characterisation of compactness is most convenient.
\begin{enumerate}[label=\textbf{(\alph*)}]
  \item If $A$ is open or $B$ is open, then $A+B$ is open.
  \item If $A$ is closed and $B$ is compact, then $A+B$ is closed.
  \item If $A$ and $B$ are compact, then $A+B$ is compact.
\end{enumerate}
\exinfo{This exercise is taken from the Analysis I \& II examination of 15 August 2019
(Prof.\ Peter S.\ Jossen), Part B, Problem 2, whose first three parts ask for the definitions of
open, closed and compact.}
\exsol{sol:sumset_open_closed_compact}
\end{exercise}

% Source: exercises_2024/mock.pdf (Analysis II mock examination, Spring Semester 2024,
% Prof. Joaquim Serra), Box Problem 16, p. 3
% Extractor: Claude Opus 5 (High)
% The whole section is about what continuity preserves, and every result in it runs in the image
% direction. This is the one-line demonstration that the preimage direction is not among them.
% Referenced from ex:box_proper_map_not_diffeomorphism in chapter 15, which needs exactly this.
\begin{exercise}[Box question: a compact set with a non-compact preimage]
\label{ex:box_preimage_of_compact_not_compact}
Give an explicit example of a continuous, nonconstant map $g : \mathbb{R}^2 \to \mathbb{R}^2$ and
a compact set $K \subseteq \mathbb{R}^2$ such that $g^{-1}(K)$ is \emph{not} compact.
\exinfo{This exercise is taken from the mock examination for Analysis II, Spring Semester 2024
(Prof.\ Joaquim Serra), Box Problem 16, where only the final answer was graded.}
\exsol{sol:box_preimage_of_compact_not_compact}
\end{exercise}
