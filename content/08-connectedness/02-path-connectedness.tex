...
The set $U$ is \newterm{simply connected} \germanfor{einfach zusammenhängend} if any two paths
in $U$ with the same endpoints are homotopic.
\end{definition}
...
The second implication is \cref{lem:path_connected_implies_connected}, and the first is immediate
from the definitions, since a homotopy between two paths presupposes that two such paths exist.
