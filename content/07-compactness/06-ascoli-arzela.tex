...
% Correction: Claude Opus 5 (Ultracode) --- the parenthetical read "the points of $K$ with
% rational coordinates will do", which fails for most compact $K$: $K = \{\sqrt2\}$ contains no
% rational point at all, and neither does a Cantor set of irrationals, so the proposed $D$ can be
% empty. The existence of $D$ is true for a different reason, given below; the theorem is
% unaffected.
Fix a countable dense subset $D = \{q_1,q_2,\dots\} \subseteq K$, which exists because $K$ is
compact and therefore totally bounded: take a finite $\tfrac1j$-net for each $j$ and let $D$ be
