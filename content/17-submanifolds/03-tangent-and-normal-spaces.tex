...
\begin{tikzpicture}[>=Latex, scale=1.1]
  ...
  % Correction: Claude Opus 5 (Max) --- these read $\partial f/\partial x_i$, but $f$ is
  % parametrized by $q$ here: the paragraph directly above writes the columns as
  % $\partial f/\partial q_i$ and \cref{def:tangent_space} directly below spans
  % $\partial_1f(q),\dots,\partial_mf(q)$. In this chapter $x$ is the ambient variable, so the
  % figure was naming derivatives in the wrong space.
  \draw[orange!90!black, thick, ->] (P) -- ++(1.1,0.35)
    node[right, font=\scriptsize] {$\partial f/\partial q_1$};
  \draw[purple, thick, ->] (P) -- ++(0.35,0.95)
    node[above, font=\scriptsize] {$\partial f/\partial q_2$};
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[scale=1.1]
  ...
  % Correction: Claude Opus 5 (High) --- the tangency point and its $e_3$ label used to be
  % drawn BEFORE this plane, and TikZ paints in source order, so the half-transparent plane
  % was laid over both: the label came out washed to a pale orange and the marker dot was
  % grey above the plane's edge and black below it. The plane is drawn first now and the
  % point last, which is also the right reading -- the point sits on the plane, not under it.
  \draw[orange!90!black, thick, fill=orange!10, fill opacity=0.5]
    (-1.7,1.3) -- (1.7,1.3) -- (2.0,1.8) -- (-1.4,1.8) -- cycle;
  % Was at (1.5,1.65), where the label ran past the plane's slanted right edge (x = 1.88 at
  % that height, against a label reaching 2.1).
  \node[text=orange!90!black, font=\small] at (1.15,1.6) {$T_{e_3}S^2$};
  \fill (0,1.3) circle (1.7pt) node[right=2pt, font=\small] {$e_3$};
\end{tikzpicture}
\end{center}
...
\begin{center}
\begin{tikzpicture}[>=Latex, scale=1.1]
  % Correction: Claude Opus 5 (High) --- the arrow was not tangent to the curve, in the one
  % figure whose whole point is that gamma'(0) IS the tangent. `plot[smooth]' makes the tangent
  % at an interior control point parallel to the chord of its neighbours, so at (0.4,-0.2) the
  % curve ran along (1.4,0.9)-(-0.8,0.6) = (2.2,0.3), slope 0.14 -- nearly flat, since the old
  % second point (-0.8,0.6) sat level with the fourth. The arrow was drawn at slope 0.83 and
  % visibly crossed the curve. Dropping that point to (-0.8,-1.2) puts p on a genuinely rising
  % stretch: the chord is now (2.2,2.1), slope 0.95, and the arrow below is parallel to it.
  \draw[blue!70!black, thick] plot[smooth] coordinates
    {(-1.8,-0.8) (-0.8,-1.2) (0.4,-0.2) (1.4,0.9) (2.2,0.1)};
  \coordinate (P) at (0.4,-0.2);
  \fill (P) circle (1.7pt) node[below, font=\small] {$p=\gamma(0)$};
  \draw[orange!90!black, thick, ->] (P) -- ++(0.85,0.81);
  % Set clear of both curve and arrow: everything below y = 0.5 for x < 0.8.
  \node[text=orange!90!black, font=\small] at (0.35, 0.78) {$\gamma'(0)$};
\end{tikzpicture}
\end{center}
...
% Correction: Claude Opus 5 (Ultracode) --- the span was written $\{0\}\times\mathbb{R}$, which
% names a subset of $\mathbb{R}^2$; the normal line lives in $\mathbb{R}^3$. Same ambiguity the
% ainote on ex:topologists_sine_curve corrects, where $\{0\}$ was adjoined to a subset of
% $\mathbb{R}^2$ and had to become $\{(0,0)\}$.
So, $N_{e_3}S^2 = \operatorname{span}\{e_3\} = \{(0,0)\}\times\mathbb{R}$.
\end{example}
...
\begin{tikzpicture}[>=Latex, scale=1.1]
  ...
  % Correction: Claude Opus 5 (High) --- `right' puts the label hard against the dot, and the
  % leading minus of $-e_3$ then merged into the 1.7pt marker: at reading size the south pole
  % label was indistinguishable from the north pole's $e_3$, which turns the picture of a
  % normal *line* into two copies of the same point. 2pt of clearance is enough.
  \fill (0,1.3) circle (1.7pt) node[right=2pt, font=\small] {$e_3$};
  \fill (0,-1.3) circle (1.7pt) node[right=2pt, font=\small] {$-e_3$};
\end{tikzpicture}
\end{center}
...
\begin{exercise}[Spherical coordinates \textnormal{(important)}]
\label{ex:spherical_coordinates_diffeomorphism}
...
\end{enumerate}
\exinfo{This exercise is Problem 7.6 of Problem Sheet 7, Analysis II, Spring Semester 2026.}
\exsol{sol:spherical_coordinates_diffeomorphism}
\end{exercise}

% Source: old_exams/fs2023/Prüfung.pdf (9 August 2021), Teil A, Frage 7, p. 8
% Extractor: Gemini 3.6 Flash (High)
\begin{exercise}[Quadric submanifold and tangent space basis]
\label{ex:submanifold_quadric_tangent_space}
Consider the set $M := \{(x_1,x_2,x_3) \in \mathbb{R}^3 \mid x_1 x_2 + x_2 x_3 + x_1 x_3 = 1\}$.
\begin{enumerate}[label=\textbf{(\alph*)}]
  \item Show that $M$ is a two-dimensional smooth submanifold of $\mathbb{R}^3$.
  \item Find an explicit basis of the tangent space $T_p M$ at the point $p = (-1, 2, 3) \in M$.
\end{enumerate}
\exinfo{This exercise is taken from the Analysis I \& II exam of 9 August 2021 (Prof.\ Giovanni Felder), Part A, Problem 7.}
\exsol{sol:submanifold_quadric_tangent_space}
\end{exercise}

% Source: exercises_2024/mock.pdf (Analysis II mock examination, Spring Semester 2024,
% Prof. Joaquim Serra), Exercise 21, p. 4
% Extractor: Claude Opus 5 (High)
% Kept as a box question, the form the source uses: only the final formula was graded. It is the
% n=3 case of prop:gradient_orthogonality_level_sets applied to G(x,x_4) := x_4 - phi(x), and its
% value is that it is set one dimension past the picture, where the familiar surface-in-R^3
% formula can no longer be read off a drawing. The official key's first display is wrong; see the
% AI-Note beside the solution.
\begin{exercise}[Box question: the normal to a graph in $\mathbb{R}^4$]
\label{ex:box_normal_to_a_graph_in_r4}
Let $\phi : \mathbb{R}^3 \to \mathbb{R}$ be differentiable. Give the formula for the unit normal
vector $N \in \mathbb{R}^4$ to the graph of $\phi$ at a point
$X = (x_1,x_2,x_3,\phi(x)) \in \mathbb{R}^4$.
\exinfo{This exercise is taken from the mock examination for Analysis II, Spring Semester 2024
(Prof.\ Joaquim Serra), Box Problem 21, where only the final answer was graded.}
\exsol{sol:box_normal_to_a_graph_in_r4}
\end{exercise}
