...
\begin{ainote}
This section has no counterpart in Corsin's notes and is transcribed directly from the official
script (\S9.1.3, printed pp. 11--13), rather than enriching an existing tutor passage the way the
rest of this document's supplements do. The reason to include it despite that: it is the
construction that makes sense of \qt{$\mathbb{Q}$ is not complete, but $\mathbb{R}$ is} in
general --- every incomplete metric space embeds isometrically, and densely, into a complete one
--- and script Exercises 9.33--9.35 walking through it are short, self-contained, and otherwise
exactly the kind of material this chapter collects. What is \emph{not} included is the script's
own use of this construction to build $\mathbb{R}$ from $\mathbb{Q}$ (\S9.1.4): that material is
marked \qt{extra; cf.\ Grundstrukturen} by the script itself and belongs to the sibling
\emph{Grundstrukturen} project.
\end{ainote}
...
field --- which is exactly how the script uses this construction in $\S9.1.4$, taking
$X = \mathbb{Q}$.
\end{remark}
