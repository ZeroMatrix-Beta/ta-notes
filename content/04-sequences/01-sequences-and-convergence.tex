...
holds for all $n \geq N$ --- equivalently, if $\mathcal{P}(x_n)$ fails for at most finitely many
...
open sets, i.e.\ only on the topology $d_1$ or $d_2$ generates --- so equal topologies give equal
...
convergence: two visibly different metrics --- even one bounded and one not --- generate the same
convergent sequences exactly when they generate the same open sets, with no need to compare them
directly. \Cref{ex:2.5}\textbf{.2}'s three product metrics are one instance of this; so is
