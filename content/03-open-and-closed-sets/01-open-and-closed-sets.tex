...
% Supplement: lec_notes.pdf, p. 11 (Definition 9.38)
\begin{ainote}[Two things to know about this definition]
\label{ainote:ball_notation}
First, \cref{def:open_ball} is stated for $r \in \mathbb{R}$ rather than $r > 0$. Harmless here,
but loose; the two degenerate cases are spelled out in the remark below.

Second, and more likely to trip you up when reading alongside the lecture: \textbf{the lecture
notes put the arguments the other way round}, writing $B(x,r)$ where this document writes
$B_r(x)$ (Definition 9.38, p.\ 11). The two mean exactly the same set, centre $x$ and radius $r$;
only the typography differs. This document keeps $B_r(x)$, because that is what Corsin's notes
use and it is what every later chapter here is written in. The one thing to watch is that in the
lecture's ordering the centre comes \emph{first}, so $B(0,1)$ and $B_1(0)$ are the same ball
while $B(1,0)$ and $B_1(0)$ are not.
\end{ainote}
...
\begin{tikzpicture}[scale=1.2]
  ...
  \begin{scope}[xshift=4cm]
    ...
    % Correction: Claude Opus 5 (High) --- a bold "open" stood here, anchored `right' at the
    % set's edge, so it printed in the gutter between this panel and the next and read as
    % belonging to neither. It also said nothing the caption below does not. The right-hand
    % panel's annotation is kept because it does more than name the panel: it points at the
    % one boundary point where the ball fails.
...
\begin{proof}
\begin{enumerate}[label=\textbf{(\alph*)}]
  \item Let $x \in \bigcup_i U_i$. Then $x \in U_{i_0}$ for some $i_0$, and since $U_{i_0}$
    is open there is $\varepsilon>0$ with
    $B_\varepsilon(x) \subseteq U_{i_0} \subseteq \bigcup_iU_i$.
  \item Let $x \in \bigcap_{i=1}^kU_i$. For each $i$ pick $\varepsilon_i>0$ with
    $B_{\varepsilon_i}(x) \subseteq U_i$, and set
  ...
  \item Take complements and apply \textbf{(a)}, \textbf{(b)} via De Morgan:
...
bracket.
\exinfo{This exercise is taken from Linus L\"uchinger's slides (Week~2, slide~20).}
\exsol{sol:open_closed_self_test}
\end{exercise}
...
\exinfo{This exercise is taken from J\'er\^ome Paschoud's class notes (Week~2.1, p.~5).}
\end{exercise}
