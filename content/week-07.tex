...
\begin{itemize}
  \item \textbf{Chart} (\cref{def:submanifold}) --- the definition. Straighten $M$ out with a
    diffeomorphism. Almost never used directly for a concrete $M$: producing $\Phi$ by hand is
  ...
  \item \textbf{Implicit / regular value} (\cref{thm:regular_value_theorem}) --- use when $M$
    arrives as \emph{equations}: $\{x^2+y^2=R^2\}$, $\{g_1=\dots=g_k=0\}$. Check a rank
  ...
  \item \textbf{Parametrization} (\cref{thm:local_parametrization}) --- use when $M$ arrives as
    ...
    formula in a parameter. Also the one to use when you subsequently need the tangent space,
