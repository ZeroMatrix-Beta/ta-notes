...
\begin{enumerate}[label=\textbf{\arabic*.}]
  \item Compute $dx^1\wedge dx^2(v,w)$, $dx^1\wedge dx^3(v,w)$ and $dx^2\wedge dx^3(v,w)$
    directly from property (4).
  \item Compute the cross product $v \times w$.
  \item The three numbers in part 1 and the three components of $v \times w$ are the same three
    numbers, but not in the same order and not all with the same sign. Find the correspondence,
