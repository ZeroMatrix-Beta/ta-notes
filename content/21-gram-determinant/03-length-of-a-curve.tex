...
\begin{tikzpicture}[>=Latex, scale=1.0]
  % Correction: Claude Opus 5 (High) --- the polygon was not inscribed. Its vertices were a
  % second, slightly different point set from the curve's -- (0.55,0.95) against (0.6,0.9),
  % (2.0,0.1) against (1.9,0.15), (3.15,0.45) against (3.1,0.5) -- five of the seven disagreed.
  % So the marked c(t_i) sat beside the curve instead of on it, and the chords crossed the curve
  % between the marks; at c(t_4) the vertex fell below the curve's own minimum. The definition
  % directly above defines L(c) as a supremum over INSCRIBED polygons, whose vertices lie on the
  % curve by construction. Curve, chords and dots now all read from one list of points.
  \draw[thick] plot[smooth] coordinates
    {(0,0) (0.6,0.9) (1.3,0.5) (1.9,0.15) (2.5,0.75) (3.1,0.5) (3.7,1.3)};
  \node[font=\scriptsize] at (1.9,1.15) {$\operatorname{Im}(c)$};
  \draw[blue, thick]
    (0,0) -- (0.6,0.9) -- (1.3,0.5) -- (1.9,0.15) -- (2.5,0.75) -- (3.1,0.5) -- (3.7,1.3);
  % Correction: Claude Opus 5 (High) --- c(t_5) was placed `above left', which put it directly
  % under the Im(c) label at (1.9,1.15) with the two boxes touching. Sent to the other side,
  % where nothing else sits: c(t_6) is `below' and the curve there runs well under the label.
  \foreach \x/\y/\lbl/\pos in {0/0/{c(t_1)}/below,0.6/0.9/{c(t_2)}/above,1.3/0.5/{c(t_3)}/above,1.9/0.15/{c(t_4)}/below,2.5/0.75/{c(t_5)}/{above right},3.1/0.5/{c(t_6)}/below,3.7/1.3/{c(t_7)}/right} {
...
% Correction: Claude Opus 5 (Ultracode) --- the displayed quantity is the square root of
% $D\gamma\transp D\gamma$, so it is the *volume* of def:volume_gram_determinant, not the Gram
% determinant, which under this chapter's convention is the bare $D\gamma\transp D\gamma$. See
% the ainote at the definition of the term.
is exactly the square root of the Gram determinant of $D\gamma(t)$, that is, the
$1$-dimensional volume of \Cref{def:volume_gram_determinant}, the $m=1$ case of
\cref{ex:gram_m1_length}, now applied
...
\begin{exercise}[Multiple integrals]
\label{ex:curve_length_polar_and_vector_integral}
...
\begin{exercisehint}[Corsin's hint on part \textbf{(b)}]
You can parametrize the triangle as
\[D = \{(x,y)\in\mathbb{R}^2 : 0 \leq x \leq y \leq \pi\}.\]
\end{exercisehint}
\exinfo{This exercise is Problem 9.2 of Problem Sheet 9, Analysis II, Spring Semester 2026.}
\exsol{sol:curve_length_polar_and_vector_integral}
\end{exercise}
...
\begin{exercise}[Solids of revolution \textnormal{(important)}]
\label{ex:solids_of_revolution_area}
...
\begin{enumerate}[label=\textbf{(\alph*)}]
  ...
  \item \textnormal{(*)} Show that $\Omega$ is a bounded $C^1$ domain (as in
    ...
    \end{equation}
\end{enumerate}
\exinfo{This exercise is Problem 10.3 of Problem Sheet 10, Analysis II, Spring Semester 2026.}
\exsol{sol:solids_of_revolution_area}
\end{exercise}
