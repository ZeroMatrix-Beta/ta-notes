...
% Correction: Claude Opus 5 (Ultracode) --- read "each endpoint $n\pm\frac1n$ is a limit point outside
% the set", which is false for the very first one: $1+\frac11 = 2$ lies inside $B_{1/2}(2)$, since
% the $n=1$ and $n=2$ intervals overlap. The intervals separate only from $n=3$ on. Argued from
% $0$ instead, which is outside the union for every $n$.
\textbf{$\bigcup_{n\geq1}B_{1/n}(n)$.} Open as a union of open sets
(\cref{item:arbitrary_union_open}); not closed, since $0$ is a limit point of the union that does
not belong to it; unbounded, so certainly not compact.
...
% Correction: Claude Opus 5 (Ultracode) --- this paragraph read "None of the reverse implications
% hold. A space satisfying (i) is called pseudocompact, and there are pseudocompact metric spaces
% that are not compact. Inside R^n the gap closes, by thm:heine_borel." All three claims are false
% in the setting the exercise fixes: for a *metric* space pseudocompactness is equivalent to
% compactness, so there is no such example and no gap for Heine-Borel to close. Every standard
% pseudocompact-but-not-compact witness ([0,omega_1), the Mrowka Psi-spaces) is non-metrizable,
% which is exactly why the half-remembered counterexample does not apply here.
A space satisfying \textbf{(i)} is called \newterm{pseudocompact}. The chain proved above is
...
What is worth adding is that the reverse implications hold as well, so for a metric space all
four statements are equivalent and pseudocompactness is not a weaker notion at all. Given
\cref{thm:sequential_topological_compactness_equivalence} it is enough to prove
\textbf{(i)} $\Rightarrow$ \textbf{(iv)}, and the contrapositive is a direct construction.
Suppose some sequence has no accumulation point. Passing to distinct terms, the set
$S := \{x_1,x_2,\dots\}$ is then closed, and every point of $X$ has a neighbourhood meeting $S$
finitely often. Put
\[r_k := \min\Big\{\tfrac13\,d\big(x_k,\,S\setminus\{x_k\}\big),\ \tfrac1k\Big\} > 0,
  \qquad f(x) := \sum_{k\ge1} k\,\max\Big\{0,\ 1 - \tfrac{1}{r_k}d(x,x_k)\Big\}.\]
The factor $\tfrac13$ makes the balls $B_{r_k}(x_k)$ pairwise disjoint, and the cap $\tfrac1k$
makes every point of $X$ have a neighbourhood on which all but finitely many summands vanish; so
$f$ is locally a finite sum of continuous functions, hence continuous. But $f(x_k) = k$, so $f$
is unbounded and \textbf{(i)} fails. (The slicker route is the same idea packaged: $S$ is closed
and discrete, $x_k \mapsto k$ is continuous on it, and Tietze extends that to $X$.)
\end{remark}
\end{exercisesolution}
...
Take
\[A := \{(n,0) \mid n \geq 1\}, \qquad B := \{(-n,\tfrac1n) \mid n \geq 1\}.\]
...
\begin{exercisesolution}[Solution to \cref{ex:infinite_discrete_subsets_compact}]
\label{sol:infinite_discrete_subsets_compact}
% Generator: Gemini 3.7 Flash (High)
\begin{enumerate}[label=\textbf{(\alph*)}]
  \item Consider the closed unit interval $K := [0, 1] \subset \mathbb{R}$ with the Euclidean metric,
    which is compact by the Heine--Borel theorem (\cref{thm:heine_borel}).
    Define the infinite subset
    \[S := \left\{\frac{1}{n} \;\middle|\; n \in \mathbb{N}_{\geq 1}\right\} \subset [0, 1].\]
    For each $n \geq 1$, choose $r_n := \frac{1}{2}\left(\frac{1}{n} - \frac{1}{n+1}\right) = \frac{1}{2n(n+1)} > 0$.
    Then the open interval $\big(\frac{1}{n} - r_n,\, \frac{1}{n} + r_n\big)$ contains no other point of $S$.
    Hence every point of $S$ is isolated in $S$, so $S$ is a discrete subset.
    Yet $S$ contains infinitely many distinct points. (Notice that $S$ is not closed in $[0, 1]$,
    since the limit point $0 = \lim_{n\to\infty} 1/n$ belongs to $K \setminus S$.)

  \item Let $K$ be a compact metric space, and let $S \subseteq K$ be a closed discrete subset.
    We give two proofs:

    \textbf{Method 1: via open covers.}
    Since $S$ is discrete, for each point $x \in S$ there exists an open ball $B_{r_x}(x)$ such that
    $B_{r_x}(x) \cap S = \{x\}$.
    Because $S$ is closed in $K$, the complement $K \setminus S$ is open.
    The collection of open sets
    \[\mathcal{U} := \big\{B_{r_x}(x) \;\big|\; x \in S\big\} \cup \{K \setminus S\}\]
    covers all of $K$, because every point of $S$ is covered by its own ball $B_{r_x}(x)$, and every
    point not in $S$ is covered by $K \setminus S$.
    By compactness of $K$, $\mathcal{U}$ has a finite subcover:
    \[K \subseteq B_{r_{x_1}}(x_1) \cup \dots \cup B_{r_{x_m}}(x_m) \cup (K \setminus S)\]
    for finitely many points $x_1, \dots, x_m \in S$.
    Intersecting both sides with $S$:
    \[S = S \cap K \subseteq \bigcup_{i=1}^m \big(B_{r_{x_i}}(x_i) \cap S\big) \cup \big((K \setminus S) \cap S\big)
      = \bigcup_{i=1}^m \{x_i\} \cup \emptyset = \{x_1, \dots, x_m\}.\]
    Therefore $S$ is a finite set.

    \textbf{Method 2: via sequential compactness.}
    Suppose for contradiction that $S$ were infinite. Then we could choose a sequence
    $(x_n)_{n \in \mathbb{N}}$ of pairwise distinct points in $S$.
    By sequential compactness of $K$ (\cref{thm:sequential_topological_compactness_equivalence}),
    there exists a subsequence $(x_{n_k})_{k \in \mathbb{N}}$ converging to some limit $x^* \in K$.
    Since $S$ is closed in $K$, the limit must lie in $S$, so $x^* \in S$.
    Because $S$ is discrete, $x^*$ is isolated in $S$, so there exists a ball $B_\rho(x^*)$ with
    $B_\rho(x^*) \cap S = \{x^*\}$.
    However, by definition of convergence, $x_{n_k} \in B_\rho(x^*)$ for all sufficiently large $k$,
    which forces $x_{n_k} = x^*$ for all large $k$.
    This contradicts that all terms in the original sequence $(x_n)$ were chosen to be pairwise distinct.
