...
\begin{ainote}[What continuous maps do and do not preserve]
...
\begin{ainote}
Corsin never formally introduces \newterm{Lipschitz continuity}, even though a
\newterm{Lipschitz constant} is already used informally for contraction mappings
...
Lipschitz continuous $\implies$ uniformly continuous (\cref{item:uniform_continuity_compact})
$\implies$ continuous (\cref{def:continuous}): given $L$, the choice $\delta := \varepsilon/L$
works uniformly in $x$, independently of $\varepsilon$. Every continuously differentiable
function on a compact domain is Lipschitz continuous (with $L := \sup|f'|$, by the mean value
theorem), but the converse implications all fail in general --- $\delta(\varepsilon) \leq
\varepsilon^2$ for $f(x) := \sqrt x$ near $0$ decays too fast for any linear bound, so $\sqrt{\cdot}$
is uniformly continuous on $[0,\infty)$ but not Lipschitz.
\end{remark}
