...
% Corrected 2026-08-09 against lec_notes.pdf. This AI-Note previously asserted that "the course
% never formally introduces Lipschitz continuity" and that the reader "will not find it under this
% name in the lecture material". Both are false: Definition 9.55 (printed p. 15) introduces uniform
% and Lipschitz continuity together. The note is kept, because the provenance point is still worth
% making, but it now says the true thing.
% Supplement: lec_notes.pdf, p. 15 (Definition 9.55)
\begin{ainote}
Corsin's own notes use a Lipschitz constant informally, for contraction mappings
(\cref{def:contraction_mapping}, below), without ever defining the term; the definition and the
hierarchy that follow are supplied from other tutors. The lecture notes do define it, alongside
uniform continuity, in Definition 9.55 on p.\ 15, so this is a gap in the TA material rather than
in the course.
\end{ainote}
...
continuous} \germanfor{Lipschitz-stetig} if there exists $L \geq 0$, a \newterm{Lipschitz
...
% Sonnet 5 (Medium)
\begin{center}
% Correction: Claude Opus 5 (High) --- each ring label was drawn straight over its own circle
% and cut in half by it. "uniformly cont." is about 2.2cm wide, so at y = -1.2 its ends reach
% x = +-1.1, and the r = 1.5 circle passes through y = -1.2 at x = +-0.9: the arc crossed the
% text's centre line. Same for "continuous" against the r = 2.2 circle. A label near the foot
% of a circle is always at risk of this, because the arc curves up into it, so rather than
% hunt for a height that happens to work, the labels now carry a white plate that masks the
% arc behind the glyphs -- the `lbl' idiom recorded in style.md.
\begin{tikzpicture}[scale=1.0,
  lbl/.style={fill=white, fill opacity=0.85, text opacity=1, inner sep=1.2pt,
              rounded corners=1pt}]
  \draw[blue!70!black, thick] (0,0) circle (2.2);
  \node[lbl, text=blue!70!black, font=\small] at (0,-1.9) {continuous};
  \draw[orange!90!black, thick] (0,0) circle (1.5);
  \node[lbl, text=orange!90!black, font=\small] at (0,-1.2) {uniformly cont.};
  \draw[purple, thick] (0,0) circle (0.8);
  % Plated like the other two: "Lipschitz" at \small is about 1.45cm, so its ends reach x = +-0.72
  % against an arc at +-0.80 -- 0.08cm of clearance, which is not clearance. The plate costs
  % nothing and removes the question.
  \node[lbl, text=purple, font=\small] at (0,0) {Lipschitz};
  \fill[orange!90!black] (1.1,0.9) circle (1.5pt)
    node[lbl, right=1pt, text=orange!90!black, font=\scriptsize] {$\sqrt{x}$};
\end{tikzpicture}
\end{center}
...
\end{tabular}
\end{center}
\exsol{sol:continuity_classification}
\end{exercise}
...
\begin{exercise}[Lipschitz or not]
\label{ex:lipschitz_or_not}
Give an example of:
...
\end{enumerate}
\exinfo{This exercise is Problem 3.1 of Problem Sheet 3, Analysis II, Spring Semester 2026.}
\exsol{sol:lipschitz_or_not}
\end{exercise}

% Originally: content/exercise-sheets/week-03.tex, problem 3.6
% Source: exercises/Ex3_Analysis2_eng.pdf

\begin{exercise}[Multiple choice (uniform continuity)]
\label{ex:uniform_continuity_multiple_choice}
...
\end{enumerate}
\exinfo{This exercise is Problem 3.6 of Problem Sheet 3, Analysis II, Spring Semester 2026.}
\exsol{sol:uniform_continuity_multiple_choice}
\end{exercise}
...
\cref{ex:uniform_continuity_multiple_choice}(c) and (d) at once.
\end{remark}

% Originally: content/exercise-sheets/week-03.tex, problem 3.11
% Source: exercises/Ex3_Analysis2_eng.pdf

\begin{exercise}[(*) Uniform continuity and moduli of continuity]
\label{ex:moduli_of_continuity}
...
\exhint[Official hints]{\textbf{3.11.1:} define the ``worst oscillation at scale $t$''
...
\exinfo{This exercise is Problem 3.11 of Problem Sheet 3, Analysis II, Spring Semester 2026.}
\exsol{sol:moduli_of_continuity}
\end{exercise}
