...
rigid failure imaginable --- it does not even shrink distances a little --- and it \emph{still}
need not have a fixed point, even on a complete space. Note that unboundedness is doing real
work in the counterexample: on a \emph{compact} space, an isometry into itself is automatically
surjective, but even that is not enough to force a fixed point --- an irrational rotation of the
circle $S^1$ (compact, and an isometry) has none either. Fixed points of isometries are a
