...
* **Sub-part Labels:** Always use alphabetical numbering for sub-parts, items, and cases (e.g., `\textbf{(a)}`, `\textbf{(b)}`). Do NOT use numerical labels like `(1), 2)`. This applies to proof sections, lists, and TikZ nodes. **Important:** Do NOT hardcode custom labels using `\item[...]` — this applies to **both** `itemize` and `enumerate`, with no exceptions. Instead, set `\begin{enumerate}[label=\textbf{(\alph*)}]` on the environment itself and use plain `\item`; for `itemize`, use plain `\item` and put any name/label as `\textbf{name:}` at the start of the item's text. **Proof Sub-parts:** Do NOT write `Proof of (a):` or use `\item[...]`. Write sub-part proof headers using `\begin{enumerate}[label=\textbf{(\alph*)}]` with plain `\item`. When referencing a specific sub-part or custom enumerate label in prose, maintain the bold formatting (e.g., "statement \textbf{(d)}", "from \textbf{(K4)}").
  ...
  **The bold wraps the label and nothing else.** One `\textbf{...}` per label, with connectives, prepositions and punctuation left outside it: write `\textbf{(b)} and \textbf{(h)}`, not `\textbf{(b) and (h)}`; write `\textbf{(e)}.`, not `\textbf{(e).}`. A trailing period swept into the bold is the easiest version to miss. This is about *pointers* to labels; a whole verdict sentence deliberately set in bold as a heading (`\textbf{(a), (c) and (e) are true; (b) and (d) are false.}`) is a different construct and stays as it is.
  ...
  * **`\textbf{Part 1.}` paragraph headers are the same defect in disguise** and are fixed the same way: delete the headers, wrap the argument in `\begin{enumerate}[label=\textbf{(\alph*)}]` with plain `\item`, and indent the displays into the items. The solution to `ex:9.2` was carrying three of them and was converted on 2026-08-11.

  **Whose numbering is it?** All lists in this project print `(a)`, `(b)`, `(c)`; the official sheets, their hints and the old exams number their parts `1, 2, 3` and `8.5.2`-style. When you sweep prose for references, decide per site which document the pointer is aimed at:

  * **Ours takes letters.** Anything pointing at a list the reader can see on the page — "by part \textbf{(c)}", "parts \textbf{(d)} and \textbf{(f)} important", `\cref{ex:2.1} part \textbf{(e)}`, and cross-file pointers into another chapter's exercise. Source comments and `% Note:` lines count too, so that a future audit reads the same map as the text.
  * **Theirs keeps its numbers.** `\exhint[Official hints]{\textbf{4.3.3:} ...}`, "the official hint for 4.4.2", `\exinfo{... Short Problem 1, part 2}` and `% Source: ... Kurzproblem 1, part 1, p. 8`. These name a part of *their* document, and renumbering them would make the citation unverifiable. The problem-sheet table in `appendix-d-problem-sheets.tex` is an inventory of the official sheets and stays numeric throughout.
  * Where both appear in one sentence, prefer rewriting over mixing: "why the two are really a single statement" beat "why 4.4.2 and 4.4.3 are ...", in `rem:` on `ex:4.4`.
