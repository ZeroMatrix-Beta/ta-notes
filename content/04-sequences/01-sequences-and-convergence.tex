...
\germanfor{schlie\ss lich} if there exists $N \in \mathbb{N}$ such that $\mathcal{P}(x_n)$
...
\begin{proof}
The idea is that two different limits could be told apart by a small enough $\varepsilon$. Past
some index every $x_n$ lies inside the ball of radius $\varepsilon$ about $x$; if $x'$ is outside
that ball, the tail of the sequence never comes close to $x'$ and cannot converge to it as well.
\begin{center}
% Correction: Claude Opus 5 (Max) --- two changes, both from style rules already recorded here.
% (1) A `text width=6cm' node reading "Proof idea: ..." sat inside the picture, so the drawing
% was pushed off-centre inside its own \begin{center} and the picture said in words what it is
% there to show. This is the same defect removed from the Cauchy--Schwarz figure in
% 02-metric-normed-inner-product/01-structured-spaces.tex, and it is removed the same way: the
% sentence is prose above. (2) The second limit was called $y$ here while the proposition, the
% proof below it, and the triangle inequality it runs on all call it $x'$.
\begin{tikzpicture}
  \fill (0,0) circle (1.5pt) node[left] {$x$};
  \draw[green!60!black, dashed, thick] (0,0) circle (1);
  % Pushed out to radius 1.06 from 1.13 `above left': anchored that way the box's lower-left
  % corner landed at (0.6,0.8) and the circle passes through (0.6,0.8) exactly.
  \node[green!60!black, anchor=south west] at (0.75,0.75) {$\varepsilon$};

  \fill (2,0) circle (1.5pt) node[right] {$x'$};
...
\newterm{accumulation point} \germanfor{H\"aufungspunkt} of $(x_n)_{n=0}^{\infty}$ if some
...
% Correction: Claude Opus 5 (Ultracode) --- the first counterexample was indexed
% $x_{2n+1} := 1/n$, $x_{2n} := 1-1/n$, which is undefined at $n=0$ (both terms divide by zero)
% and puts $x_3 = 1/1 = 1$ outside the very space $[0,1)$ the example is set in. Reindexed on
% $k$ directly, matching the second counterexample's form; the mathematics is unchanged.
this as false in general, with two counterexamples worth recording. First, a sequence can have a
unique accumulation point without converging: on $[0,1)$, let $x_k := \tfrac1k$ for odd
$k \geq 3$ and $x_k := 1-\tfrac1k$ for even $k \geq 2$, both of which lie in $[0,1)$; the only
accumulation point is $0$, yet $(x_k)$ does not converge, since the even terms tend to
$1 \notin [0,1)$ and so leave every neighbourhood of $0$ infinitely often. Second,
...
% Correction: Claude Opus 5 (Ultracode) --- the closing clause read "such as compactness of the
% ambient space together with some control ruling out the first counterexample's
% boundary-escaping behaviour", which overstates what is needed: compactness alone suffices, and
% the escaping behaviour it points at *is* the non-compactness of $[0,1)$. Suggesting a residual
% gap where there is none is the sort of hedge a reader remembers as a real caveat.
every subsequence of a convergent sequence converges to the same limit). The converse needs one
extra hypothesis and no more: (sequential) compactness of the ambient space
(\cref{sec:compactness}). If $(x_k)$ has $x$ as its only accumulation point but does not converge
to it, some $\varepsilon>0$ leaves a subsequence outside $B_\varepsilon(x)$; compactness gives
that subsequence a convergent sub-subsequence, whose limit $y$ satisfies
$d(y,x) \geq \varepsilon$ and is a second accumulation point, a contradiction. Both
counterexamples above are simply non-compact: $[0,1)$ in the first, $\mathbb{R}$ in the second.
\end{remark}
...
the metrics directly. \Cref{ex:product_metric_spaces}\textbf{.2}'s three product metrics are one instance of this; so is
...
% Correction: Gemini 3.7 Flash (High) --- in panel (c), x_n previously converged to (1.8, 0)
% which lies on the dashed (open) boundary y=0 and thus escaped; redirected to the solid (closed)
% boundary at (1.8, 0.9) so that its limit belongs to the set and x_n remains trapped, matching the prose.
%
% Correction: Claude Opus 5 (Ultracode), found by rendering printed p. 58 --- that fix was right and
% the panel still did not show what the example above claims. Three things follow it.
%   * The escaping limit carried no mark at all while the trapped one carried the red $\times$, so
%     the figure's own grammar said the interesting half had no limit. Solid dot now means "the
%     limit lies in the set" and a hollow one "it does not" -- a distinction in shape, not hue,
%     as an e-reader needs, and the convention `06-completeness/01' and `03-unit-balls' already
%     use. Panel (a)'s corner limit becomes a solid dot for the same reason.
%   * The two limits in (c) were the wrong points. The example names its witnesses precisely:
%     not open because no ball round $(0,\tfrac12)$ fits, not closed because
%     $(\tfrac12,\tfrac1n) \to (\tfrac12,0)$. The panel escaped through the TOP edge at $x = 0.67$
%     and landed on the RIGHT edge at $y = 0.5$ -- both legitimate, neither the one written down,
%     under a sentence reading "exactly these three witnesses". In panel coordinates $[0,1]^2$ is
%     drawn as $[0,1.8]^2$, so $(0,\tfrac12) \mapsto (0,0.9)$ and $(\tfrac12,0) \mapsto (0.9,0)$,
%     and the two sequences now run to those.
%   * Panel (b) drew a sequence escaping to a corner. That is evidence the set is not CLOSED,
%     under a caption reading "open", and the example's own argument for (b) is a ball:
%     the distance from $x$ to the nearest edge is some $\varepsilon > 0$ and
%     $B_\varepsilon(x) \subseteq (0,1)^2$. It now draws that ball. Centre $(0.9,0.95)$ sits
%     $0.85$ from the nearest edge, so radius $0.7$ fits with room.
\begin{center}
\begin{tikzpicture}[>=Latex, scale=1.4]
  % Panel 1: Closed
  \begin{scope}[shift={(0,0)}]
    \draw[thick, fill=blue!5] (0,0) rectangle (1.8,1.8);
    \draw[purple, thick, dotted, ->] (0.5,0.6).. controls (0.8,1.2) and (1.2,1.3).. (1.72,1.72);
    \node[purple, font=\footnotesize, left] at (0.6,0.6) {$x_n$};
    \fill[red] (1.8,1.8) circle (2pt);
    \node[below, font=\small] at (0.9,-0.1) {(a) $[0,1]^2$ (\textbf{closed})};
  \end{scope}

  % Panel 2: Open -- the ball the example actually argues with
  \begin{scope}[shift={(2.8,0)}]
    \draw[thick, dashed, fill=blue!5] (0,0) rectangle (1.8,1.8);
    \draw[purple, thick, dashed] (0.9,0.95) circle (0.7);
    \draw[purple, thick, ->] (0.9,0.95) -- ++(0.495,0.495)
      node[midway, above left=-3pt, font=\scriptsize] {$\varepsilon$};
    \fill[purple] (0.9,0.95) circle (1.6pt);
    \node[purple, font=\footnotesize, below left=-2pt] at (0.9,0.95) {$x$};
  ...
  \begin{scope}[shift={(5.6,0)}]
    ...
    % y_n -> (1/2, 0): escapes through the open bottom edge, so its limit is not in the set.
    % Label set to the RIGHT of the curve's start: `left' at (0.45,0.85) ran the box back to
    % x = 0.09, straight into x_n's arrowhead and the solid dot on the left edge.
    \draw[orange!90!black, thick, dotted, ->] (0.62,0.72).. controls (0.75,0.45) and (0.85,0.25).. (0.88,0.09);
    \node[orange!90!black, font=\footnotesize, right] at (0.64,0.74) {$y_n$};
    \draw[red, thick, fill=white] (0.9,0) circle (2pt);

    % x_n -> (0, 1/2): the closed left edge, so its limit is in the set. Stopped at x = 0.20
    % rather than 0.11 so the arrowhead points AT the dot instead of sitting on top of it.
    \draw[purple, thick, dotted, ->] (1.1,1.35).. controls (0.75,1.22) and (0.42,1.06).. (0.20,0.93);
    \node[purple, font=\footnotesize, right] at (1.12,1.37) {$x_n$};
    \fill[red] (0,0.9) circle (2pt);

    \node[below, font=\small] at (0.9,-0.1) {(c) $[0,1]\times(0,1)$ (\textbf{neither})};
  \end{scope}

  \node[font=\footnotesize, anchor=north] at (3.7,-0.55)
    {\tikz\fill[red] (0,0) circle (2pt); \ limit lies in the set \qquad
     \tikz\draw[red, thick, fill=white] (0,0) circle (2pt); \ limit lies outside it};
\end{tikzpicture}
\end{center}
...
\begin{enumerate}[label=\textbf{(\alph*)}]
  ...
  \item This is \newterm{uniform convergence} \germanfor{gleichm\"assige Konvergenz}!
...
\begin{tikzpicture}[>=Latex, scale=1.4,
  ...
  % Correction: Claude Opus 5 (Max) --- this arrow measured nothing. It ran (1.3,0.5)--(1.3,1.2),
  % and at $x=1.3$ the curve is $1.0+0.3\sin(3.25) = 0.9675$, so the two dotted rails sit at
  % $0.6175$ and $1.3175$. The length $0.7$ was right but the whole arrow was displaced $0.1175$
  % down: its lower head sat BELOW the tube and its upper head inside it, so the one dimension
  % line in the picture spanned neither rail. Pinned to the rails.
  %
  % The label has to take the white plate: the arrow's midpoint is $f(1.3)$ by construction, so
  % any label at `midway' lands on the curve, and moving it left or right does not help, since
  % the curve descends from $0.968$ at $x=1.3$ to $0.798$ at $x=1.55$ and climbs to $1.148$ at
  % $x=1.05$ -- it crosses a \footnotesize box on either side.
  \draw[orange!90!black, <->, thick] (1.3, 0.6175)
    -- node[lbl, right, font=\footnotesize] {$\frac{2\varepsilon}{3}$} (1.3, 1.3175);
