...
bottom-left arrow failing backwards --- both partial derivatives exist at the origin and $f$ is
not even continuous there. The other two failures, with their counterexamples, are collected in
...
as before --- vary one entry, freeze the rest.

\begin{ainote}[Why this is worth a subsection]
Taking derivatives with respect to a matrix argument can look like a formal exercise with no
purpose. Linus spends part of a session arguing the opposite, and the argument is worth
recording: \textbf{it is how neural networks are trained.} The parameters of such a network are
matrices; the function being minimised sends those matrices to a single number measuring how
badly the network fits the data; and training consists of computing the gradient with respect
to those matrices and stepping a little way against it, because $-\nabla f$ points in the
direction of steepest decrease. Every step of that loop is the material of this chapter.
\end{ainote}
...
gradient with respect to $A$ is the residual correlated against the inputs --- both entirely
explicit, and both computed by nothing beyond partial differentiation and the chain rule.
\end{aiexample}

\begin{ainote}[A notational caution]
Linus flags one point that trips people up. Since $f$ above has domain
...
$\partial_vf = \Jac f\cdot v$ of \cref{sec:jacobi_matrix} still holds --- it is just being read
