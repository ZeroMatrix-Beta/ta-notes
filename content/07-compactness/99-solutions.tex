...
\begin{ainote}
Part \textbf{(b)} is also script Exercise 9.76, which the script proves without invoking
connectedness at all --- worth recording as a second route, since it is more elementary. Fix
$x \in U$ and $y \notin U$, and let $\gamma(t) := x+t(y-x)$, $t\in[0,1]$, be the segment between
them. The point $\gamma(t_*)$ with $t_* := \sup\{t\in[0,1] : \gamma(t)\in U\}$ cannot lie in $U$
(openness of $U$ would let $t_*$ be pushed further out), yet is a limit of points
$\gamma(t_k) \in U$ with $t_k \nearrow t_*$ --- a convergent, hence Cauchy, sequence in $U$ whose
limit escapes $U$. This avoids \cref{lem:path_connected_implies_connected} entirely, at the cost
of only working in $\mathbb{R}^n$ (the segment needs the vector space structure), whereas the
connectedness proof above generalises to any connected, locally connected ambient space.
\end{ainote}
...
\begin{enumerate}[label=\textbf{(\alph*)}]
  ...
  \item Both hypotheses of \cref{thm:ascoli_arzela} hold on the compact set $[0,1]$, so some
    ...
    uniform one --- its derivative $f_k'(x) = kx^{k-1}$ has $\lVert f_k'\rVert_\infty = k$,
    growing without bound. And we saw that no subsequence of $(x^k)$ converges. What matters is
