...
The precise definitions will follow later: metric spaces in \cref{def:metric_space} just below,
inner products and norms only in Week 3 (\cref{def:inner_product}, \cref{def:norm}). You will
see that an \textbf{inner product} $\langle\cdot,\cdot\rangle$ induces a \textbf{norm} by the
formula
\[\|x\| := \sqrt{\langle x, x\rangle}, \qquad x \in X.\]
Furthermore, a \textbf{norm} induces a \textbf{metric} by
\[d(x,y) := \|x - y\|, \qquad x, y \in X.\]
That both formulas really do produce a norm and a metric --- the triangle inequality being the
only non-obvious part --- is \cref{prop:induced_norm_and_metric} in Week 3, where the tools to
prove it are available.

\begin{ainote}
The two blocks that follow supplement Corsin's outline with material from other tutors, and both
work with inner products and the Cauchy--Schwarz inequality \emph{before} this document
formally defines them, because Corsin's own notes place that material in Week 3
(\cref{sec:cauchy_schwarz}) and reordering his chapters is not something these notes do. Read
them as a preview: nothing later depends on them, and \cref{sec:cauchy_schwarz} restates
everything from scratch. \Cref{prop:cauchy_schwarz_geometric_proof} below and
...
\cref{rem:two_proofs_cauchy_schwarz} points out.
\end{ainote}
