...
% Source: old_exams/fs2023/Probeprfg1.pdf (Probeprüfung Analysis II, undated), Aufgabe 11, p. 3
% Extractor: Claude Opus 5 (High)
% The paper names no lecturer; see old-exam-mining.md.
% Kept here rather than filed as a one-variable problem: the point is not the integral but the
% choice of the two vectors in the space of continuous functions, which is the first place in the
% document where the abstract inner product does something the Euclidean one cannot.
\begin{exercise}[An integral inequality from Cauchy--Schwarz]
\label{ex:integral_reciprocal_inequality}
Let $a < b$ and let $f : [a,b] \to \mathbb{R}_{>0}$ be continuous and strictly positive. Show
that
\[\int_a^b f(x)\,dx \int_a^b \frac{1}{f(x)}\,dx \ \geq\ (b-a)^2 ,\]
and determine for which $f$ equality holds.
\exhint{The space $C([a,b],\mathbb{R})$ carries the inner product
$\langle g,h\rangle := \int_a^b g(x)h(x)\,dx$. Only the choice of the two vectors is left to
find.}
\exinfo{This exercise is taken from the mock examination \emph{Probepr\"ufung Analysis II}
(undated) in \texttt{old\_exams/fs2023/}, Problem 11. That paper names no lecturer, and asks only
for the inequality; the equality case is added here.}
\exsol{sol:integral_reciprocal_inequality}
\end{exercise}
...
\begin{exercise}[Cauchy--Schwarz]
\label{ex:cauchy_schwarz_statement}
...
linearly dependent.
\exinfo{This exercise is Problem 3.7 of Problem Sheet 3, Analysis II, Spring Semester 2026.}
\exsol{sol:cauchy_schwarz_statement}
\end{exercise}
...
\begin{exercise}[Inner product and norm]
\label{ex:inner_product_induced_norm}
...
\exinfo{This exercise is Problem 3.8 of Problem Sheet 3, Analysis II, Spring Semester 2026.}
\exsol{sol:inner_product_induced_norm}
\end{exercise}

% Originally: content/exercise-sheets/week-03.tex, problem 3.9
% Source: exercises/Ex3_Analysis2_eng.pdf

\begin{exercise}[All norms are equivalent in $\mathbb{R}^n$]
\label{ex:norm_equivalence_rn}
...
$\tilde f$.}
\exinfo{This exercise is Problem 3.9 of Problem Sheet 3, Analysis II, Spring Semester 2026.}
\exsol{sol:norm_equivalence_rn}
\end{exercise}
...
\exsol{sol:polynomials_infinite_dimensional}
\end{exercise}

\begin{remark}
This is exactly the hypothesis \cref{ex:norm_equivalence_rn} needs and cannot do without.
...
\begin{exercise}[Hilbert--Schmidt norm of the composition]
\label{ex:hilbert_schmidt_submultiplicative}
...
\exinfo{This exercise is Problem 3.10 of Problem Sheet 3, Analysis II, Spring Semester 2026.}
\exsol{sol:hilbert_schmidt_submultiplicative}
\end{exercise}

% Originally: content/week-04.tex, \exercisesheet{4}
% Source: exercises/Ex4_Analysis2_eng.pdf

\begin{exercise}[$p$-norms \textnormal{(semi-important; parts \textbf{(d)} and \textbf{(f)}
important)}]
\label{ex:p_norms_unit_balls}
...
\exinfo{This exercise is Problem 4.3 of Problem Sheet 4, Analysis II, Spring Semester 2026.}
\exsol{sol:p_norms_unit_balls}
\end{exercise}

% Originally: content/exercise-sheets/week-04.tex, problem 4.4 --- never previously built
% Source: exercises/Ex4_Analysis2_eng.pdf

\begin{exercise}[$p$-means \textnormal{(optional)}]
\label{ex:p_means_monotonicity}
...
\exinfo{This exercise is Problem 4.4 of Problem Sheet 4, Analysis II, Spring Semester 2026.}
\exsol{sol:p_means_monotonicity}
\end{exercise}
