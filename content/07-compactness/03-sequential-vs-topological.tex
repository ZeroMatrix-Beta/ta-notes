...
\begin{tikzpicture}[>=Latex, scale=1.5]
  ...
  % Correction: Claude Opus 5 (Max) --- both labels started at x = 1.3, inside the blob. The
  % outline runs through the control point (1.8,0.2) and crosses x = 1.68 at y = 0.4 and x = 1.80
  % at y = 0.1, i.e. squarely inside each label's own height, so the `very thick' boundary printed
  % through both. Moved clear of the blob (its rightmost bulge is about x = 1.87) and given the
  % leaders they never had, so that each label is attached to what it names rather than merely
  % matching it in hue --- which style.md forbids relying on by itself.
  \node[green!60!black, anchor=west] at (2.05, 0.4) {\footnotesize accumulation point};
  \draw[green!60!black, thin] (2.02, 0.4) -- (1.16, 0.26);
  \node[purple, anchor=west] at (2.05, 0.1) {\footnotesize subsequence $x_{n_k} \to x$};
  \draw[purple, thin] (2.02, 0.08) -- (0.98, 0.02);
\end{tikzpicture}
\end{center}
...
% Correction: Claude Opus 5 (Ultracode) --- the estimate was run with the ball $B_{r(x)}(x)$,
% which yields only $R(y) \geq r(x) - d(x,y)$ and, after halving,
% $r(y) \geq \tfrac12 r(x) - \tfrac12 d(x,y)$. That does NOT give the Lipschitz bound claimed on
% the next line: rearranged it reads $r(x)-r(y) \leq \tfrac12 r(x) + \tfrac12 d(x,y)$, with a
% stray $\tfrac12 r(x)$ left over, and it is not even sharp at $x=y$ --- the sign that a factor
% has been lost. It is $R$, not $r$, that is Lipschitz on the nose, so the radius has to run up
% to $R(x)$ rather than stopping at its halfway point. The conclusion was right; the derivation
% did not reach it.
$r = \tfrac12 R$; it is $R$ that the argument bounds, and the factor $\tfrac12$ is carried
through at the end.

Fix $x,y \in X$ and let $\rho < R(x)$ be any radius with $B_\rho(x) \subseteq U_i$ for some $i$.
The triangle inequality gives
\[B_{\rho-d(x,y)}(y) \subseteq B_{\rho}(x) \subseteq U_i
\qquad\Longrightarrow\qquad R(y) \geq \rho-d(x,y),\]
trivially so when $\rho \leq d(x,y)$, the ball on the left being empty. Letting
$\rho \uparrow R(x)$, which is legitimate because $R(x)$ is the supremum of such radii and is
finite, gives $R(y) \geq R(x)-d(x,y)$; exchanging $x$ and $y$ gives the reverse inequality, so
$\lvert R(x)-R(y)\rvert \leq d(x,y)$. Halving,
...
% Kept as a worked example rather than an exercise: it is the second, harder application of the
% Lebesgue number above, and the two-regime split it turns on is the transferable part.
\begin{example}[Locally Lipschitz on a compact space means Lipschitz]
\label{ex:locally_lipschitz_is_lipschitz}
Call $f : X \to \mathbb{R}$ \newterm{locally Lipschitz} \germanfor{lokal Lipschitz-stetig} if
every $x_0 \in X$ admits an open neighbourhood $U_{x_0}$ and a constant $L_{x_0} > 0$ with
\[\lvert f(x)-f(y)\rvert \le L_{x_0}\,d(x,y) \qquad \text{for all } x,y \in U_{x_0}.\]
The constant is allowed to depend on the point, and on a general space it must: on $(0,1]$ the
function $f(x) := 1/x$ is locally Lipschitz and not Lipschitz, the local constants blowing up as
$x_0 \to 0$. If $X$ is compact, that cannot happen.

\textbf{Claim.} If $(X,d)$ is compact and $f : X \to \mathbb{R}$ is locally Lipschitz, then $f$ is
Lipschitz (\cref{def:lipschitz_continuous}).

The neighbourhoods $\{U_{x_0}\}_{x_0 \in X}$ cover $X$, so compactness extracts a finite subcover
$U_{x_1},\dots,U_{x_N}$. Put $L := \max_{1\le i\le N} L_{x_i}$, which is finite because the maximum
is now over finitely many indices, and let $\delta > 0$ be a Lebesgue number for that finite
subcover (\cref{def:lebesgue_number}). Fix $x,y \in X$ and split on how far apart they are.

\begin{enumerate}[label=\textbf{(\alph*)}]
  \item \textbf{Nearby points, $d(x,y) < \delta$.} The ball $B_\delta(x)$ lies inside some
    $U_{x_i}$, and both $x$ and $y$ belong to it. The local estimate on $U_{x_i}$ therefore
    applies to this pair, giving $\lvert f(x)-f(y)\rvert \le L_{x_i}d(x,y) \le L\,d(x,y)$.
  \item \textbf{Distant points, $d(x,y) \ge \delta$.} Here no local estimate is available, and
    none is needed. A locally Lipschitz function is continuous, so $f$ is bounded on the compact
    $X$, say $\lvert f\rvert \le M$. Then
    \[\lvert f(x)-f(y)\rvert \le 2M = \frac{2M}{\delta}\,\delta \le \frac{2M}{\delta}\,d(x,y).\]
\end{enumerate}

Consequently $f$ is Lipschitz with constant $\max\{L,\,2M/\delta\}$.

% Feedback: Gemini 3.1 Pro (2026-08-11) --- called a didactic highlight: it says exactly why the
% proof is run in two regimes and where the finite subcover alone fails. Do not trim it back into
% the proof body.
\begin{remark}[Why two regimes, and why neither alone suffices]
The finite subcover alone does \emph{not} finish the argument, and this is the step most solutions
skip. It bounds the local constants, but a pair $x,y$ lying in \emph{no} common $U_{x_i}$ is
never reached by any of them. The Lebesgue number is exactly what repairs that: it converts
\qt{some member of the cover} into \qt{a uniform radius around every point}, so that every
sufficiently close pair is covered by one estimate. The far regime is then free, because over a
fixed distance a bounded function cannot vary fast.
\end{remark}
\exinfo{This example is taken from the Analysis I \& II examination of winter 2018/2019
(Prof.\ Manfred Einsiedler), Problem 9, where it is set as a proof exercise. The counterexample on
$(0,1]$ and the two-regime commentary are added here.}
\end{example}
...
\begin{center}
% Correction: Claude Opus 5 (Ultracode), found by rendering printed p. 91 --- the figure drew one
% dashed rectangle round $A$, called it $\mathcal{V}$, and captioned it "$\mathcal{V}$ covers $K$,
% so also $A$". Three things were wrong at once. The rectangle spans $x \in [-1,1.2]$,
% $y \in [-0.8,0.8]$ while $K$ reaches $x = \pm 2$ and $y = 1.2$: it visibly does not cover $K$,
% which is the "open cover that does not cover" this document has been caught by before.
% $\mathcal{V}$ occurs nowhere in the proposition or the proof, which use $\mathcal{U}$ and
% $\mathcal{U}' = \mathcal{U} \cup \{X\setminus A\}$. And the implication ran backwards: the proof
% enlarges a cover of $A$ into one of $K$, not the other way round.
% The redraw shows the enlargement itself, which is the step the remark below names
% ("enlarge the cover, then discard the extra set"): $\mathcal{U}$ round $A$, and $X\setminus A$
% -- drawn even-odd, clipped to $K$, since only its part inside $K$ does any work -- supplying
% everything else.
\begin{tikzpicture}[scale=1.2,
  lbl/.style={fill=white, fill opacity=0.8, text opacity=1, inner sep=1.2pt,
              rounded corners=1pt}]
  % X \ A, drawn first so nothing later is painted over.
  \begin{scope}
    \clip plot[smooth cycle, tension=0.7] coordinates {(-1.5, -1) (-2, 0.5) (0, 1.2) (2, 0.5) (1.5, -1.2)};
    \fill[green!12, even odd rule] (-2.4,-1.6) rectangle (2.4,1.6)
      (0, 0.5) -- (-0.6, 0) -- (0, -0.5) -- (0.6, 0) -- cycle;
  \end{scope}

  % Compact set K
  \draw[very thick] plot[smooth cycle, tension=0.7] coordinates {(-1.5, -1) (-2, 0.5) (0, 1.2) (2, 0.5) (1.5, -1.2)};
  % Correction: Claude Opus 5 (Max) --- `above left' at (-1.8,0.5) put the box across x = -2.1
  % to -1.8, and the blob's outline passes through its own control point (-2,0.5), inside that
  % span: the very thick boundary ran through the letter. Set inside the blob instead, where the
  % nearest boundary is at x = -2.0 and the dashed rectangle's left edge at x = -1.0.
  \node[lbl, font=\large] at (-1.35, 0.45) {$K$};

  % Closed set A
  \draw[thick, fill=gray!25] (0, 0.5) -- (-0.6, 0) -- (0, -0.5) -- (0.6, 0) -- cycle;
  \node[font=\large] at (0, 0) {$A$};

  % The added set. Its true boundary is the diamond's edge, already drawn above. Set on the LEFT,
  % where the gap between the rectangle (left edge x = -1.0) and K (x = -1.9 at this height) is
  % widest; on the right it landed on the rectangle's own corner.
  \node[lbl, text=green!35!black, font=\small] at (-1.5, -0.5) {$X\setminus A$};

  % The cover of A that the proof starts from.
  \draw[dashed, blue] (-1, 0.8) rectangle (1.2, -0.8);
  % Inside the rectangle's top-right corner rather than outside it: anchored there, the label
  % straddled K's own thick outline, which passes through this corner.
  \node[lbl, text=blue, font=\small, anchor=north east] at (1.12, 0.72) {$\mathcal{U}$};

  % Correction: Claude Opus 5 (Max) --- `right' at (1.2,0.5) sent this running rightwards from
  % the rectangle's edge, and the blob's control point (2,0.5) sits 0.8 units into it at exactly
  % its vertical centre, so the very thick outline crossed the caption. Below the whole figure
  % instead: the blob's lowest reach is about y = -1.35 and nothing else is down there.
  % One line, not two: the second line restated the sentence directly below the figure and drove
  % the picture's box wider than the blob itself.
  \node[blue, font=\small, below] at (0, -1.5)
    {$\mathcal{U}$ covers $A$; adding $X\setminus A$ covers $K$};
\end{tikzpicture}
\end{center}
...
% Supplement: lec_notes.pdf, p. 21 (Corollary 9.71)
% Generator: Claude Opus 5 (High) --- the converse of the proposition above, and the half of
% Heine--Borel that costs nothing and holds in every metric space. The document had it only
% inside \cref{thm:heine_borel}, where it looks like a fact about R^n, which it is not.
\begin{proposition}[A compact subset is always closed]
\label{prop:compact_implies_closed}
Let $(X,d)$ be a metric space and $K \subseteq X$ compact. Then $K$ is closed in $X$.
\end{proposition}

\begin{proof}
Let $(x_n) \subseteq K$ converge to some $x \in X$; by \cref{prop:open_closed_via_sequences} it
suffices to show $x \in K$. Compactness gives a subsequence $x_{n_k} \to y$ with $y \in K$
(\cref{thm:sequential_topological_compactness_equivalence}). But a subsequence of a convergent
sequence converges to the same limit, so $y = x$ by uniqueness of limits
(\cref{prop:uniqueness_of_limits}), and therefore $x \in K$.
\end{proof}

\begin{remark}[This is the cheap half of Heine--Borel]
\label{rem:compact_closed_bounded_cheap_half}
Together with \cref{lem:sequentially_compact_totally_bounded}, which gives boundedness, this
proposition says that in \emph{every} metric space a compact set is closed and bounded. That
direction is short and completely general, and it is the one \cref{thm:heine_borel} gets for
free. What Heine--Borel adds, and what is special to $\mathbb{R}^n$, is the converse, and
\cref{ex:heine_borel_fails} shows how badly the converse can fail elsewhere: there is a metric
space with a closed bounded set that is not compact.

The pairing with \cref{prop:closed_subset_of_compact} is worth noticing too. Compactness passes
\emph{down} to closed subsets and \emph{up} to the property of being closed, so within a compact
ambient space \qt{compact} and \qt{closed} are the same word.
\end{remark}
...
% Source: old_exams/fs2023/Probeprfg3.pdf (Probeprüfung Analysis II, 31 January 2023), Aufgabe 5(a)--(b), p. 2
% Extractor: Gemini 3.7 Flash (High)
% The paper names no lecturer; see old-exam-mining.md.
% Infinite discrete versus closed discrete subsets: a compact space can accommodate an infinite
% discrete set (which must necessarily accumulate outside itself), but cannot hold a closed one.
\begin{exercise}[Infinite discrete versus closed discrete subsets of a compact space]
\label{ex:infinite_discrete_subsets_compact}
Let $(X, d)$ be a metric space. A subset $S \subseteq X$ is called \newterm{discrete} if every
point $p \in S$ is an isolated point of $S$, that is, there exists $r > 0$ such that
$B_r(p) \cap S = \{p\}$.
\begin{enumerate}[label=\textbf{(\alph*)}]
  \item Give an example of a compact metric space $K$ containing an infinite discrete subset $S \subset K$.
  \item Prove that if $K$ is a compact metric space, then every \emph{closed} discrete subset
    $S \subseteq K$ must be finite.
\end{enumerate}
\exinfo{This exercise is taken from the mock examination \emph{Probepr\"ufung Analysis II} of
31 January 2023, Problem 5(a)--(b). That paper names no lecturer.}
\exsol{sol:infinite_discrete_subsets_compact}
\end{exercise}
