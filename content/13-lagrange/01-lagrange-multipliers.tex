...
\begin{tikzpicture}[>=Latex, scale=1.3]
  ...
  % Correction: Claude Opus 5 (Ultracode) --- the dotted line through $p$ was drawn
  % $(-0.3,-0.3)$ to $(1.0,-0.6)$, direction $(1.3,-0.3)$ at $-13.0^\circ$, and the gradient was
  % drawn $(0.75,-0.2)$ at $-14.9^\circ$: the two were $1.9^\circ$ apart. So the panel whose whole
  % claim is that $\nabla T(p)$ *has* a tangential component drew it lying along the tangent,
  % i.e. with no normal component at all -- the opposite failure from the one at $q$, and just as
  % wrong. The dotted line was not the tangent either: at $p$ the radius is
  % $p/|p| = (0.614,-0.789)$, so the tangent is $\tau = (0.789,0.614)$, and
  % $(1.3,-0.3)\cdot(0.614,-0.789) = 0.562 \neq 0$.
  % Redrawn from $\tau$ and $n$: the dotted line is now the true tangent, and
  %   $\nabla T(p) := 0.55\,n + 0.50\,\tau = (0.732,-0.127)$,
  % which stands $42.3^\circ$ off the radius and $47.7^\circ$ off the tangent -- visibly neither.
  % Its tangential half is drawn as well, since that is the thing the caption names.
  \coordinate (P) at (0.35, -0.45);
  \draw[orange!90!black, dotted, thick] (-0.163, -0.849) -- (0.863, -0.051);
  \draw[orange!90!black, thick, ->] (P) -- ++(0.732, -0.127)
    node[below right, font=\small] {$\nabla T(p)$};
  \draw[orange!60!black, thin, dashed] (0.7445, -0.143) -- (1.082, -0.577);
  \draw[orange!90!black, thick, ->] (P) -- ++(0.3945, 0.3070);
  \fill[orange!90!black] (P) circle (1.8pt) node[left, font=\footnotesize] {$p$};
  \node[orange!90!black, font=\tiny, align=center, anchor=north] at (0.4, -1.48)
    {tangential component $\neq 0$\\$\implies$ not a local extremum};
\end{tikzpicture}
\end{center}
...
sufficient}. Plenty of points have a perpendicular gradient without being extrema at all. On the
earth, the coldest place has a perpendicular gradient just as the hottest does, and so does many
a saddle. What the condition delivers is a shortlist of \emph{candidates}. Deciding which of them
are maxima, which minima, and which neither is a separate step, and in practice it is done by
evaluating $T$ at each and comparing.

Note also that $S^2$ is the \newterm{level set} \germanfor{Niveaumenge} $g(x) = 0$ for
...
radial direction, and therefore perpendicular to $S^2$. In general, the gradient is perpendicular
to the level sets.
\end{remark}
...
The constraint actually differentiated in the Lagrangian below is nonetheless the squared one.
See \cref{ex:xyz_on_ball} for the same $|x|$ versus $|x|^2$ slip in the exercise.
\end{ainote}
...
$\mathbb{R}^n$. This is made precise by \cref{thm:regular_value_theorem}, which also explains
\cref{prop:constrained_optimization} geometrically: $\nabla f(x_0)$ must be normal to the tangent
space of the constraint set at a constrained extremum.
\end{remark}
...
% Feedback: Gemini 3.1 Pro (2026-08-11) --- called a perfect, memorable counterexample for the
% constraint qualification, which is the hypothesis students drop. Keep the cuspidal cubic; a
% smoother constraint would not make the point.
...
the \newterm{cuspidal cubic} \germanfor{Neilsche Parabel}. Since $y^2 \geq 0$ forces
...
point that matters. Geometrically, $M$ has a \emph{cusp} at the origin. It is not a smooth curve
there, so it has no tangent line, and \qt{$\nabla f$ perpendicular to the constraint} has nothing
...
The standing assumption above, that $(\nabla g_1,\dots,\nabla g_k)$ are linearly independent, is
...
price} \germanfor{Schattenpreis} of the constraint. A large $\lvert\lambda\rvert$ says the
constraint is expensive, and $\lambda = 0$ says it is not binding at all.
\end{remark}
...
Read the two underbraces. Differentiating in the $x_i$ reproduces the geometric condition, namely
$\nabla f$ parallel to the $\nabla g_j$, with no tangential component left. Differentiating in
...
the end. It is scaffolding, not part of the answer. That does not make it meaningless, and
\cref{rem:lagrange_multiplier_sensitivity} above says what it measures.
\end{remark}

% Source: Corsin Nick/Class Notes/Week 5.pdf, pp. 5--6
% Reclassified from exercise+exercisesolution to a worked example by Claude Opus 5 (Extra),
% 2026-08-13, together with the two blocks below it. All three were exercises whose full solutions
% were printed immediately underneath (this one carried "% Kept inline: solved directly in Corsin
% Nick/Class Notes/Week 5.pdf, pp. 5--6", so the arrangement did mirror the source), and all three
% are the chapter's demonstrations of the method rather than questions: the running text above
% points forward to this one, the figure below is keyed to its eight boundary candidates, and
% ex:second_order_test_on_circle later reopens ex:extrema_on_circle by name. Nothing is lost as
% practice, because the chapter's eight genuine exercises still sit at the end with their
% solutions in 99-solutions.tex. Corsin's own order and wording are untouched.
% Transition: Claude Opus 5 (Extra)
Three worked examples follow, in increasing order of how much of the work is not the multiplier
rule. The first has a constraint set with an interior, so it needs the unconstrained theory as
well; the second is the smallest non-trivial case and gets reopened later with the second-order
test; the third has to be turned into a constrained problem before any of this applies.

\begin{example}[Extrema of $xyz$ on the unit ball]
...
\[\underbrace{y^2z^2 = x^2z^2}_{y^2 = x^2} = \underbrace{x^2y^2}_{y^2 = z^2} \qquad \text{(if any coordinate is 0, so are the others)},\]
and therefore $x^2 = y^2 = z^2$.
...
$xyz<0$, since flipping the sign of one coordinate flips the sign of the product. They are the
three-variable analogue of a saddle.
\end{example}
...
% Moved here from ~180 lines further down, where it sat after two unrelated exercises: the eight
% marked vertices are exactly the boundary candidates found in \cref{ex:xyz_on_ball} above.
\begin{center}
\begin{tikzpicture}[>=Latex, scale=1.4]
  ...
  % Correction: Claude Opus 5 (Max) --- the cube was not centred on the sphere. The top face was
  % the bottom face translated by (0,1.2), so the eight vertices had centroid
  %   ((-0.6+0.6+0.8-0.4)/4 + 0, (-0.5-0.5+0.2+0.2)/4 + 0.6) = (0.1, 0.45),
  % not the origin. Two consequences, both visible: the whole cube floated above the equator
  % ellipse, and B3 = (0.8,1.4) sat at radius 1.612 and B4 = (-0.4,1.4) at 1.456, against a
  % sphere drawn with radius 1.4 -- so two of the eight dots captioned "critical points on S^2"
  % were printed OUTSIDE the sphere's own outline, which no projection of a point of S^2 can be.
  % Every vertex is shifted by (-0.1,-0.45), which moves the cube's centre onto the sphere's and
  % changes nothing else.
  %
  % Correction: Claude Opus 5 (Ultracode) --- the cube then disagreed with its own axes. Its three
  % edge vectors were $(1.2,0)$, $(0.2,0.7)$ and $(0,1.2)$, so $e_1 \mapsto (1,0)$ and
  % $e_3 \mapsto (0,1)$ matched the drawn $x$ and $z$ axes, but $e_2 \mapsto (0.2,0.7)$ points up
  % and to the RIGHT while the $y$ axis above is drawn towards $(-0.8,1.0)$, up and to the LEFT:
  % $54.6^\circ$ apart, so the picture was not a projection of the frame it labels.
  % Rebuilt from the axes instead of adjusted. Taking half-edges
  %   $e_1 \mapsto (0.6,0)$, $e_2 \mapsto 0.35\,(-0.8,1.0) = (-0.28,0.35)$, $e_3 \mapsto (0,0.6)$,
  % a vertex is $\pm(0.6,0) \pm(-0.28,0.35) \pm(0,0.6)$, i.e. screen
  %   $x = \pm 0.6 \mp 0.28$, $y = \pm 0.35 \pm 0.6$.
  % Bottom face $A$ is $e_3 = -1$, top face $B$ is $e_3 = +1$, so $B_i = A_i + (0,1.2)$ still, and
  % $A_1A_2A_3A_4$ is still a parallelogram: $A_1A_2 = (1.2,0) = 2e_1$, $A_2A_3 = (-0.56,0.7) = 2e_2$.
  % Screen radii, all inside 1.4 as they must be:
  %   A1 1.002  A2 1.295  A3 0.406  A4 0.915  B1 0.406  B2 0.915  B3 1.002  B4 1.295,
  % and the centroid is $(0,0)$ exactly, as before.
  \coordinate (A1) at (-0.32, -0.95);
  \coordinate (A2) at (0.88, -0.95);
  \coordinate (A3) at (0.32, -0.25);
  \coordinate (A4) at (-0.88, -0.25);

  \coordinate (B1) at (-0.32, 0.25);
  \coordinate (B2) at (0.88, 0.25);
  \coordinate (B3) at (0.32, 0.95);
  \coordinate (B4) at (-0.88, 0.95);
...
\begin{example}[Extrema on a circle]
...
% Source: Jérôme Paschoud/Notizen/Woche 5.2 Optimierung I.pdf, p. 3
% Generator: Gemini 3.6 Flash (High)
% Moved up from below the second-order test, next to the other first-order drill it belongs with.
\begin{example}[Largest rectangle in an ellipse]
...
\medskip
Nothing here is constrained until we make it so. The first step is to name the unknowns and read
the constraint off the geometry. Let the vertices of the rectangle in the four quadrants be $(\pm x, \pm y)$ with $x, y \geq 0$. The area of the rectangle is $f(x,y) = 4xy$.
...
Note that the answer does not depend on the shape of the ellipse in the way one might expect: the
optimal rectangle occupies the fraction $2AB/(\pi AB) = 2/\pi$ of the ellipse's area whatever $A$
and $B$ are, because the substitution $(x,y) = (Au,Bv)$ turns the whole problem into the same
question about the unit circle.
\end{example}


% Originally: content/week-05.tex, end of the chapter

% Supplement: Analysis_II_Script_v1.pdf, p. 73
% Moved here from between thm:spectral_theorem_optimization and cor:courant_fischer_extreme
% (Claude Opus 5, Extra, 2026-08-13); solution now in 99-solutions.tex.
\begin{exercise}[Antipodal points maximise distance on the sphere]
\label{ex:antipodal_max_distance}
Let $n \geq 2$. Prove that two points $x,y \in S^{n-1}$ have maximal distance $|x-y|$ if and
only if $x = -y$. Consider the function $(x,y) \mapsto |x-y|^2$ on
$S^{n-1}\times S^{n-1} \subset \mathbb{R}^{2n}$.
\exhint{The constraint set is a product of two spheres, so there are two constraints, and
\cref{prop:constrained_optimization} asks for their gradients to be linearly independent before
it will produce any multipliers at all.}
\exsol{sol:antipodal_max_distance}
\end{exercise}
...
\begin{exercise}[Extrema on a closed ball, inside and on the boundary \textnormal{(important)}]
\label{ex:lagrange_ball_extrema}
...
\begin{enumerate}[label=\textbf{(\alph*)}]
  ...
  \item Among all the points you have found, clarify at which points the maximum/minimum of $f$
    is attained.
\end{enumerate}
\exinfo{This exercise is Problem 5.2 of Problem Sheet 5, Analysis II, Spring Semester 2026.}
\exsol{sol:lagrange_ball_extrema}
\end{exercise}

% Originally: content/week-06.tex, \exercisesheet{6}, problem 6.10
% Source: exercises/Ex6_Analysis2_eng.pdf

\begin{exercise}[Extrema on a sphere cut by a plane \textnormal{(important)}]
\label{ex:lagrange_sphere_plane}
...
\exinfo{This exercise is Problem 6.10 of Problem Sheet 6, Analysis II, Spring Semester 2026.}
\exsol{sol:lagrange_sphere_plane}
\end{exercise}

% Originally: content/week-06.tex, \exercisesheet{6}, problem 6.11
% Source: exercises/Ex6_Analysis2_eng.pdf

\begin{exercise}[Closest point on a hyperbolic paraboloid \textnormal{(important)}]
\label{ex:closest_point_hyperbolic_paraboloid}
Find the points on the submanifold
\[M = \{(x,y,z) \in \mathbb{R}^3 : z = x^2-y^2\}\]
that are closest to the point $(0,0,1)$.
\exinfo{This exercise is Problem 6.11 of Problem Sheet 6, Analysis II, Spring Semester 2026.}
\exsol{sol:closest_point_hyperbolic_paraboloid}
\end{exercise}

% Source: old_exams/fs2023/Prüfung.pdf (9 August 2021), Teil B, Aufgabe 4, p. 16
% Extractor: Gemini 3.6 Flash (High)
% Correction: Claude Opus 5 (High) --- Flash cited "the January 2022 exam, Problem 4 of Part B".
% Problem 4 of Part B of that exam is about harmonic numbers and the Gamma function; this problem is
% Aufgabe 4 of Part B of the *August 2021* exam. The problem number was right, the exam was not.
\begin{exercise}[Extrema of a linear form on an ellipsoid]
\label{ex:lagrange_ellipsoid_linear}
Let $a_1, a_2, a_3 > 0$ and let $b = (b_1, b_2, b_3)\transp \in \mathbb{R}^3 \setminus \{0\}$. Find the maximum and minimum values of the linear function $f(x) := b_1 x_1 + b_2 x_2 + b_3 x_3$ on the ellipsoid
\[E := \left\{x \in \mathbb{R}^3 \ \middle|\ \frac{x_1^2}{a_1^2} + \frac{x_2^2}{a_2^2} + \frac{x_3^2}{a_3^2} = 1\right\},\]
and determine the points of $E$ at which these extreme values are attained.
\exinfo{This exercise is taken from the Analysis I \& II exam of 9 August 2021 (Prof.\ Giovanni Felder), Part B, Problem 4.}
\exsol{sol:lagrange_ellipsoid_linear}
\end{exercise}

% Source: old_exams/WS22_Prüfung.pdf (24 January 2022), Teil A, Frage 8, p. 9
% Extractor: Claude Opus 5 (High)
\begin{exercise}[Minimising entropy on the simplex]
\label{ex:entropy_simplex_minimum}
Let $n \ge 1$ and let $A := \{x \in (0,1]^n \mid x_1 + \dots + x_n = 1\}$. Consider
\[f : A \to \mathbb{R}, \qquad f(x) := \sum_{i=1}^n x_i \log(x_i).\]
\begin{enumerate}[label=\textbf{(\alph*)}]
  \item Show that $f$ has exactly one critical point $p$ in the interior of $A$, and determine
    $f(p)$.
  \item Prove that $f$ attains its minimum on $A$ at $p$.
\end{enumerate}
\exinfo{This exercise is taken from the Analysis I \& II exam of 24 January 2022 (Prof.\ Giovanni Felder), Part A, Problem 8.}
\exsol{sol:entropy_simplex_minimum}
\end{exercise}

% Source: old_exams/august2025.pdf (Prof. Laura Kobel-Keller), Analysis II examination,
% August 2025, Frage 4, printed p. 4
% Extractor: Claude Opus 5 (High)
\begin{exercise}[Single choice: a constrained problem with no critical point]
\label{ex:kobel_lagrange_single_choice}
Consider a function $f$ of three variables, of which all we know is that
\[\nabla f(x,y,z) = \begin{pmatrix} e^x \\ 2y \\ 2z \end{pmatrix},\]
together with the constraint $g(x,y,z) := ay^2 + bz^2 - \pi = 0$ for parameters
$a,b \in \mathbb{R}\setminus\{0\}$. Exactly one of the following is correct. Which?
\begin{enumerate}[label=\textbf{(\alph*)}]
  \item Independently of the values of $a$ and $b$, $f$ has no extremal point under the given
    constraint.
  \item The constraint defines a compact set in space independently of the values of $a$ and $b$,
    so both the constrained minimum and the constrained maximum have to be attained.
  \item In the case $a = b$, the constrained minimum is attained.
  \item Without further knowledge of $f$, no assertion about the constrained optimisation problem
    is possible.
\end{enumerate}
\exinfo{This exercise is taken from the Analysis II examination of August 2025
(Prof.\ Laura Kobel-Keller), Question 4.}
\exsol{sol:kobel_lagrange_single_choice}
\end{exercise}

% Source: old_exams/august2025.pdf (Prof. Laura Kobel-Keller), Analysis II examination,
% August 2025, Aufgabe 13, printed p. 11
% Extractor: Claude Opus 5 (High)
\begin{exercise}[Two constraints that leave nothing for Lagrange to do]
\label{ex:kobel_sphere_meets_cylinder}
Determine the maximal value of
\[f : \mathbb{R}^3 \to \mathbb{R}, \qquad f(x,y,z) := x^2 + y + z^2\]
subject to the two constraints $x^2+y^2+z^2 = 1$ and $x^2+y^2 = 1/2$.
\exinfo{This exercise is taken from the Analysis II examination of August 2025
(Prof.\ Laura Kobel-Keller), Problem 13, a box problem where only the final answer was graded.}
\exsol{sol:kobel_sphere_meets_cylinder}
\end{exercise}
