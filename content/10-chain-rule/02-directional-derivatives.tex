...
\begin{tikzpicture}[>=Latex, scale=1.0]
  ...
  \begin{scope}
    ...
    % Correction: Claude Opus 5 (Max) --- the second ray was (0.15,0.15)--(2.35,2.35), i.e. the
    % line $y=x$, which passes through (1.15,1.15) and so misses $x_0=(1.15,1.0)$ by 0.15. The
    % $v$ arrow below leaves $x_0$ in direction $(1,1)$, so its extension is $y=x-0.15$, not
    % $y=x$: the panel captioned "pick a direction and walk along that ray" drew a ray that does
    % not pass through the point being walked from, 0.15cm clear of it and parallel. The
    % horizontal ray was always right, since it is drawn at $y=1.0$.
    \draw[gray!55, dashed] (-0.15,1.0) -- (2.75,1.0);
    \draw[gray!55, dashed] (0.30,0.15) -- (2.50,2.35);
...
\begin{itemize}
  ...
  \item \textbf{Watch whether $v \mapsto \partial_vf(x_0)$ is linear.} If $f$ were differentiable
    ...
    continuity, and it is precisely the test that settles \cref{ex:directional_derivatives_computation}\textbf{(b)} below.
\end{itemize}
\end{aiexample}
...
\label{ex:directional_derivatives_computation}
...
\exsol{sol:directional_derivatives_computation}
\end{exercise}

\begin{remark}
Corsin's hint, \textit{recall the linearity of the differential}, is the whole of
\Cref{ex:directional_derivatives_computation}(b): a directional derivative must be \textbf{linear} in $v$, and $v_1^2$ is not.
\end{remark}
...
\label{ex:derivatives_gradient_polar_computation}
...
\exsol{sol:derivatives_gradient_polar_computation}
\end{exercise}
