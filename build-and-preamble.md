...
⚠️ **Four chapters break this rule and are known to** (audited 2026-08-13, Ch. 13 split 2026-09-09):
**4** (*Sequences and Convergence* / `01-sequences-and-convergence.tex`), **14** (*Convexity*),
**15** (*The Inverse Function Theorem*) and **16** (*The Implicit Function Theorem*). They were left
alone deliberately, because the repair is not free: promoting a subsection to a section moves every
theorem number in the chapter from `N.a.M` to `N.a.M` / `N.b.M`, and that ripples through every
`\cref` target in the printed document. **It is the user's call, not a tidy-up.**

Chapter 13 was split on the user's instruction into three sections: `01-lagrange-multipliers.tex`
(first-order theory and first-order exercises), `02-second-order-conditions.tex` (second-order test on
the tangent space), and `03-spectral-theorem.tex` (the spectral theorem via constrained optimization),
putting the spectral theorem directly into the table of contents. Chapters 15 and 16 have no
subsections to promote, so for those the only repair is the other one: let the chapter absorb the
section.
