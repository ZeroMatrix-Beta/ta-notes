...
$d_0(x,y) = \sqrt{2-2\langle x,y\rangle}$. The geodesic metric described above --- arc length
along a great circle, the solid blue curve in both panels --- is instead
...
$x \neq y$: the chord always cuts inside the sphere, so $d_0(x,y) \leq d_1(x,y)$, with equality
only in the limit $y \to x$. This is the sharpest instance in the chapter of a general warning
--- a subset inherits a metric from its ambient space, but that inherited metric need not be the
one a geometer studying the subset \emph{intrinsically} would choose.
\end{remark}
