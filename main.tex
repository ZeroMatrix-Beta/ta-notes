...
% NB: deliberately NOT \newtheorem{theorem}{Theorem}[section]. The counter resets per
% *chapter* (see \@addtoreset below) and the printed number is built by hand in
% \thetheorem, which appends the section letter. An automatic per-section reset would
% restart the arabic part inside a chapter and print e.g. "6.a.1", "6.b.1", "6.c.1"
% instead of a single run 6.a.1 ... 6.c.17.
%
% This comment used to say that each content file hard-codes its own \thetheorem prefix to
% stay in step with "Prof. Biran's handwritten notes". That was imported wholesale from a
% different repository (a linear-algebra project) and never described this document: no
% file here overrides \thetheorem, and none should -- doing so would nail a wrong chapter
% number into a topic that has since moved chapters more than once.
...
% Generator: Claude Sonnet 5 (High) --- part-level framing prose, new content, not tied to any
% tutor source.
This short part collects what the rest of the course leans on from \emph{Analysis I}, together
with a few habits of visualization used throughout: how to picture sets and functions of several
variables, and Young's inequality, whose role stays invisible until norms and inner products
enter in the next part.
...
% Generator: Claude Sonnet 5 (High) --- part-level framing prose, new content.
Ordinary calculus on $\mathbb{R}^n$ depends on very little about $\mathbb{R}^n$ itself --- only
that it carries a notion of distance. This part abstracts that notion into a metric space and
develops the topological vocabulary (open, closed, compact, connected) needed to make sense of
convergence and continuity in a setting general enough to include function spaces alongside
$\mathbb{R}^n$. Completeness and compactness, in particular, are the two finiteness-type
properties that almost every existence argument later in the course quietly leans on.
...
% Generator: Claude Sonnet 5 (High) --- part-level framing prose, new content.
With a topology in hand, this part extends differentiation from one variable to several: the
differential as a linear approximation, the chain rule along paths and through compositions, and
Taylor's theorem as the systematic way to improve that approximation order by order. Almost
everything from here on --- optimization, the inverse and implicit function theorems, vector
calculus --- is built on the single definition of differentiability given at the start of it.
...
% Generator: Claude Sonnet 5 (High) --- part-level framing prose, new content.
Differentiability turns the search for extrema into algebra: unconstrained critical points are
found by setting a gradient to zero, and classified by the sign of a Hessian. This part builds
that machinery, extends it to extrema constrained to a level set via Lagrange multipliers, and
closes with convexity --- the one hypothesis under which a critical point is automatically a
\emph{global} extremum, not merely a candidate that still needs checking.
...
% Generator: Claude Sonnet 5 (High) --- part-level framing prose, new content.
Two questions drive this part: when can an equation be solved locally for one variable in terms
of the others (the implicit function theorem), and when is a map locally invertible (its close
relative, the inverse function theorem)? Both answers hinge on the same linear-algebra
hypothesis --- invertibility of a Jacobian --- and both feed directly into the definition of a
submanifold: a set that locally looks like $\mathbb{R}^m$ sitting inside $\mathbb{R}^n$, which is
exactly the kind of object the rest of the course learns to integrate over.
...
% Generator: Claude Sonnet 5 (High) --- part-level framing prose, new content.
Having spent several parts differentiating, the course now turns to integration in several
variables. Jordan measure gives a rigorous notion of volume; the change of variables formula
generalizes substitution; and the Gram determinant measures how much a parametrization distorts
area or volume --- the tool that makes integrating over a curved submanifold, rather than a flat
region of $\mathbb{R}^n$, meaningful.
...
% Generator: Claude Sonnet 5 (High) --- part-level framing prose, new content.
The course closes by giving the classical vector operators --- gradient, divergence, curl --- and
the classical integral theorems --- Green, the divergence theorem, Stokes --- a single unifying
language: differential forms and the exterior derivative. Every theorem in this part turns out to
be the same identity, $\int_M d\omega = \int_{\partial M}\omega$, stated once and specialized
several different ways --- the payoff for the topological, differential and integration
machinery built across the whole document.
