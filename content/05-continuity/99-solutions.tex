...
\begin{exercisesolution}[Solution to \cref{ex:continuity_of_distance}]
\label{sol:continuity_of_distance}
...
\cref{ex:product_metric_spaces} part \textbf{(b)} we have $d_2 \leq 2d_1$ and $d_2 \leq \sqrt2\,d_3$, so
...
Continuity survives too, being topological, and by \cref{ex:product_metric_spaces} the three product metrics
...
\begin{exercisesolution}[Solution to \cref{ex:continuity_of_composition}]
\label{sol:continuity_of_composition}
% Generator: Claude Opus 5 (High)
...
\label{sol:continuity_classification}
...
\begin{exercisesolution}[Solution to \cref{ex:lipschitz_or_not}]
\label{sol:lipschitz_or_not}
% Generator: Claude Opus 5 (High)
...
\begin{exercisesolution}[Solution to \cref{ex:problems_at_origin}]
\label{sol:problems_at_origin}
% Generator: Claude Opus 5 (High)
...
\begin{exercisesolution}[Solution to \cref{ex:uniform_continuity_multiple_choice}]
\label{sol:uniform_continuity_multiple_choice}
% Generator: Claude Opus 5 (High)
...
\begin{exercisesolution}[Solution to \cref{ex:moduli_of_continuity}]
\label{sol:moduli_of_continuity}
% Generator: Claude Opus 5 (High)
\begin{enumerate}[label=\textbf{(\alph*)}]
  ...
  \item \textbf{(Bonus 2) Convexity is exactly what the argument needed.} Repeat part \textbf{(b)}
    verbatim, now reading $|x-y|$ as the Euclidean norm on $\mathbb{R}^n$ rather than the absolute
    value on $\mathbb{R}$: the only property of an interval that was used is that the segment from
    $x$ to $y$ lies inside $X$, and that is the definition of convexity. The intermediate point is
...
$|f(n+1)-f(n)| = 2n+1$ is unbounded, so $\omega_f(1) = +\infty$. Parts \textbf{(b)} and
\textbf{(c)} rule this out by
