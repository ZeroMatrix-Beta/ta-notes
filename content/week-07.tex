...
This chapter's subject is submanifolds of $\mathbb{R}^n$: the regular value and local
parametrization theorems that characterize them, and their tangent and normal spaces. The
exercise sheet is at the end of the chapter, in \cref{sec:week07_solutions}.
...
\begin{itemize}
  \item An \textbf{implicit} description $F : \mathbb{R}^n \to \mathbb{R}^\ell$ cuts $M$ out by
    ...
    rank $=\ell$ means $DF_x$ is \textbf{surjective} --- the $\ell$ constraints are independent,
  ...
  \item A \textbf{parametrization} $f : \mathbb{R}^m \to \mathbb{R}^n$ sweeps $M$ out by $m$
    ...
    means $Df_x$ is \textbf{injective} --- the $m$ parameters move in independent directions,
    none of them wasted. Here $\dim M = m$.
\end{itemize}
Same phrase, opposite condition. A wide matrix is never injective and a tall one is never
surjective, so there is no ambiguity in practice --- but it is worth checking which side you are
on before writing down what \qt{full rank} is supposed to give you.
\end{importantremark}
...
\[\Jac F(x,y) = (2x,\ 2y)\]
has full rank --- here $\ell = 1$, so by \cref{rem:full_rank_two_meanings} full rank means
$DF_{(x,y)} : \mathbb{R}^2 \to \mathbb{R}$ is \textbf{surjective}, i.e.\ simply that the row
$(2x,2y)$ is nonzero --- if $x \neq 0$ or $y \neq 0$, that is,
...
set, we get a submanifold. When it is not, we get \emph{no information} --- the level set may
still be a submanifold, or it may not. It is worth seeing a case where it is not.

Let $F(x,y) := xy$ and consider $C := F^{-1}\{0\}$, the union of the two coordinate axes.
Here $\Jac F(x,y) = (y,\ x)$, which is nonzero --- hence surjective --- at every point of $C$
...
does not. Compare \cref{ex:constraint_qualification_fails} in Week 5, where the constraint set
$\{y^2=x^3\}$ had a cusp at exactly the point where $\nabla g$ vanished --- the same phenomenon,
seen through the Lagrange multiplier lens one chapter earlier.
\end{remark}
\end{aiexample}
...
equivalent --- but they are not equally convenient, and choosing badly is what makes these
exercises feel hard.

\begin{itemize}
  \item \textbf{Chart} (\cref{def:submanifold}) --- the definition. Straighten $M$ out with a
    diffeomorphism. Essentially, the first $m$ coordinates of the chart (the non-vanishing ones) define the $m$-dimensional submanifold. Almost never used directly for a concrete $M$: producing $\Phi$ by hand is
    painful. Its role is theoretical, and it is what the other two are proved from.
  \item \textbf{Implicit / regular value} (\cref{thm:regular_value_theorem}) --- use when $M$
    arrives as \emph{equations}: $\{x^2+y^2=R^2\}$, $\{g_1=\dots=g_k=0\}$. This relies on the same underlying idea as the chart definition, but defining the diffeomorphisms implicitly. Check a rank
    condition on $\Jac F$ and you are done, with no parametrization ever written. This is
    normally the cheapest route.
  \item \textbf{Parametrization} (\cref{thm:local_parametrization}) --- use when $M$ arrives as
    a \emph{picture you can sweep out}: a graph, a surface of revolution, a curve given by a
    formula in a parameter. This is often the most intuitive approach, explicitly building the submanifold using parameters. Also the one to use when you subsequently need the tangent space,
    since $\partial_1f,\dots,\partial_mf$ hand you a basis directly
...
\emph{surjective} in the first case and \emph{injective} in the second --- the shape of the
matrix decides. And neither theorem yields a negative: if the rank condition fails, you have
learned nothing, and must argue separately (see \cref{ex:cross_not_submanifold}).
\end{remark}
...
\begin{enumerate}[label=\textbf{\arabic*.}]
  \item The Jacobian matrix has full rank:
\[ \Jac f(x) = \begin{pmatrix} \id_{n\times n} \\ 2x\transp \end{pmatrix} \]
    $n$ at every $x$ --- a tall matrix now, so by \cref{rem:full_rank_two_meanings} this means
    $Df_x$ is \textbf{injective}, which is clear from the $\id_{n\times n}$ block alone.
...
\begin{remark}[Parametrized optimization, revisiting \cref{ex:6.9}]
...
= \begin{pmatrix} \cos\theta\cos\varphi \\ \cos\theta\sin\varphi \\ -\sin\theta \end{pmatrix} \times \begin{pmatrix} -\sin\theta\sin\varphi \\ \sin\theta\cos\varphi \\ 0 \end{pmatrix}
= \begin{pmatrix} \sin^2\theta\cos\varphi \\ \sin^2\theta\sin\varphi \\ \sin\theta\cos\theta\cos^2\varphi + \sin\theta\cos\theta\sin^2\varphi \end{pmatrix}
...
7.1 & important & ``First think of a diffeomorphism $(a,b) \leftrightarrow \mathbb{R}$ and
  adjust it such that $(a,b)=(0,1)$.'' \\[3pt]
7.2 & unmarked & --- \\[3pt]
7.3 & unmarked & --- \\[3pt]
7.4 & unmarked & --- \\[3pt]
7.5 & important & ``This is a great exercise to help with understanding of IFT.'' \\[3pt]
7.6 & important & ``You will need these coordinates very often over the next weeks.'' \\[3pt]
7.7 & semi-important & ``In \textbf{1)}, only the M\"obius strip should be enough.'' \\[3pt]
7.8 & semi-important & ``Do \textbf{1)} and \textbf{2)}. The others will be in the lecture.'' \\[3pt]
7.9 & unmarked & --- \\
\end{tabular}
\end{center}
\end{ainote}
