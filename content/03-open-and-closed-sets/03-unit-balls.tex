...
to all $p \geq 1$, is sketched in \cref{ex:p_norms_unit_balls}\textbf{.1}, where the square reappears as the
...
\begin{exercise}[Multiple choice (closure)]
\label{ex:closure_multiple_choice}
...
\end{enumerate}
\exhint[Official hints]{\textbf{(b):} compare with \cref{ex:metric_space_examples}.3. \textbf{(d):} play with a set
of three points (a triangle).}
\exinfo{This exercise is Problem 2.2 of Problem Sheet 2, Analysis II, Spring Semester 2026.}
\exsol{sol:closure_multiple_choice}
\end{exercise}

% Originally: content/exercise-sheets/week-02.tex, problem 2.3
% Source: exercises/Ex2_Analysis2_eng.pdf, p. 1

\begin{exercise}[Multiple choice (distance from a set)]
\label{ex:distance_from_set_multiple_choice}
...
\exinfo{This exercise is Problem 2.3 of Problem Sheet 2, Analysis II, Spring Semester 2026.}
\exsol{sol:distance_from_set_multiple_choice}
\end{exercise}

% Originally: content/exercise-sheets/week-02.tex, problem 2.4
% Source: exercises/Ex2_Analysis2_eng.pdf, p. 1

\begin{exercise}[Boundary, Interior etc.]
\label{ex:boundary_interior_closure_drill}
...
\begin{center}
\begin{tabular}{ll ll}
\textbf{(a)} & $Y = [0,1]$ & \textbf{(b)} & $Y = \mathbb{Q}$ \\
\textbf{(c)} & $Y = \emptyset$ & \textbf{(d)} & $Y = (0,1)$ \\
\textbf{(e)} & $Y = [-1,1) \setminus \{0\}$ & \textbf{(f)} & $Y = [0,\infty)$ \\
\textbf{(g)} & $Y = \{0\}$ & \textbf{(h)} &
  $Y = \left\{\tfrac{1}{n} \mid n \in \mathbb{N}\setminus\{0\}\right\}$
\end{tabular}
\end{center}
\exinfo{This exercise is Problem 2.4 of Problem Sheet 2, Analysis II, Spring Semester 2026.}
\exsol{sol:boundary_interior_closure_drill}
\end{exercise}
