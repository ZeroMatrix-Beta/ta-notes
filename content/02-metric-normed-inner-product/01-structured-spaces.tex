...
directly --- especially its chapters 13--14, where the $|\cdot|$ convention is used densely ---
that a bare $|\cdot|$ there and a bare $\|\cdot\|$ here can both denote \qt{the} Euclidean norm.
\end{ainote}
...
A very important example of a norm on the space of real $n \times m$ matrices $\mathbb{R}^{n \times m}$ is the \newterm{Hilbert--Schmidt norm} (also known as the Frobenius norm). It is defined by
\[
\|M\|_2 := \sqrt{\operatorname{Tr}(M\transp M)},
\]
where $\operatorname{Tr}$ denotes the trace of a matrix.
For a $2 \times 3$ matrix $M = \left(\begin{smallmatrix} a & b & c \\ d & e & f \end{smallmatrix}\right)$, we have
\[
M\transp M = \begin{pmatrix} a & d \\ b & e \\ c & f \end{pmatrix} \begin{pmatrix} a & b & c \\ d & e & f \end{pmatrix} = \begin{pmatrix} a^2+d^2 & * & * \\ * & b^2+e^2 & * \\ * & * & c^2+f^2 \end{pmatrix}.
\]
The trace is the sum of the diagonal elements, so $\|M\|_2 = \sqrt{a^2+b^2+c^2+d^2+e^2+f^2}$, which shows that this norm simply corresponds to the standard Euclidean norm of the matrix entries viewed as a vector in $\mathbb{R}^{6}$ (or generally $\mathbb{R}^{nm}$). The inner product inducing this norm is $\langle A, B \rangle := \operatorname{Tr}(A\transp B)$.
\end{example}
