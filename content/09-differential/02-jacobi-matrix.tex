...
\begin{tikzpicture}[>=Latex, scale=1.3]
  ...
  % Correction: Claude Opus 5 (High) --- this label was `above left' of the arrow tip at
  % (0.107,1.307), so with scale=1.3 its box ran from x = -1.36 to 0.14 at y = 1.70 to 1.95,
  % and the $y$ axis label sits at (0,1.9): the black y printed inside the closing bracket of
  % $\gamma'(\pi/4)$. Set beside the arrow instead, which clears the axis label in y.
  \draw[orange!90!black, ultra thick, ->] (P) -- ++(-0.6, 0.6)
    node[left=2pt, font=\small] {$\gamma'(\pi/4)$};
\end{tikzpicture}
\end{center}
...
\end{enumerate}
\exsol{sol:ai_differential_linearization}
\end{aiexercise}
...
\label{ex:derivative_computation_exponential}
...
\exsol{sol:derivative_computation_exponential}
\end{exercise}
...
\exsol{sol:piecewise_differentiability_tf}
\end{exercise}

% Source: old_exams/august2025.pdf (Prof. Laura Kobel-Keller), Analysis II examination,
% August 2025, Frage 3, printed p. 3
% Extractor: Claude Opus 5 (High)
\begin{exercise}[Single choice: which condition gives differentiability at the origin?]
\label{ex:kobel_differentiable_single_choice}
Let $f$ be a function defined on the unit ball $B_1(0) \subseteq \mathbb{R}^n$. Exactly one of the
following guarantees that $f$ is differentiable at the origin. Which?
\begin{enumerate}[label=\textbf{(\alph*)}]
  \item All partial derivatives of $f$ exist on the unit ball.
  \item There exists an \emph{arbitrary} map $L$ such that
    \[\lim_{h\to0}\frac{\lVert f(h)-f(0)-L(h)\rVert}{\lVert h\rVert} = 0.\]
  \item There exists a \emph{linear} map $L$ such that
    $f(h)-f(0)-L(h) = o\big(\lVert h\rVert^2\big)$.
  \item All partial derivatives of $f$ are bounded on the unit ball.
\end{enumerate}
\exinfo{This exercise is taken from the Analysis II examination of August 2025
(Prof.\ Laura Kobel-Keller), Question 3.}
\exsol{sol:kobel_differentiable_single_choice}
\end{exercise}
