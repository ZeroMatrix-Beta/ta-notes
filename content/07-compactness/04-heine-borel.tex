...
% The two examples below were moved up to here by Claude Opus 5 (Extra), 2026-08-13. They had been
% sitting between ex:heine_borel_fails and its own solution, forty lines apart, which split an
% exercise from its answer and interrupted a run that ends in rem:boundedness_not_topological
% ("what ex:heine_borel_fails has *just* demonstrated"). Both belong here instead: they are
% failures of compactness in the standard metric, which is what the example above is about, and
% the second one compares itself to that example by name.
...
\exsol{sol:kobel_compactness_characterisation}
\end{exercise}
