...
- [x] **C5a** `week-02.tex` — proof that `C⁰([0,1])` is closed asserts its crux:
...
- [x] **C5b** `week-02.tex` — Lebesgue-number proof conflates `r := ½·sup{…}`
...
- [x] **C6a** `week-03.tex` — "S is connected" for the topologist's sine curve
...
- [x] **C7** `week-04.tex` — "a nonzero linear map cannot decay along its own
