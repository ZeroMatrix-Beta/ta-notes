...
the \newterm{supremum norm} (\germanterm{Supremumsnorm})
...
(\cref{sec:why_compactness_matters}) --- the same point already made for the supremum metric in
\cref{ex:sup_metric_is_metric}. Convergence in this norm is exactly \emph{uniform} convergence.
...
Finally $f$ is continuous, being a uniform limit of continuous functions --- the standard
...
$\lVert f_k\rVert_\infty = 1$, so the sequence is bounded --- indeed it lies on the unit sphere.
...
uniformly continuous (\cref{def:uniformly_continuous}) --- it is continuous on a compact
interval --- but there is no single $\delta$ that works for all of them at once. Ruling that out
is precisely what the next definition does.
\end{remark}
\end{aiexample}
...
(\germanterm{gleichgradig stetig}) if
...
\begin{itemize}
  \item \textbf{Continuity} of one $f$: $\delta$ may depend on $\varepsilon$ \emph{and on the
    point} $x$.
  \item \textbf{Uniform continuity} of one $f$: $\delta$ may depend on $\varepsilon$ only ---
    one $\delta$ for all points.
  \item \textbf{Equicontinuity} of a family: $\delta$ may depend on $\varepsilon$ only --- one
    $\delta$ for all points \emph{and all members of the family}.
\end{itemize}
...
\begin{ainote}
Linus states the theorem without proof, remarking that it is \qt{mostly just used in proofs}.
The sketch below is included because the mechanism --- a diagonal argument --- explains why
\emph{both} hypotheses are needed, which the statement alone does not. The details are omitted.
\end{ainote}
...
legitimate but no single one handles all of $D$; taking the \emph{diagonal} sequence --- the
$j$-th member of the $j$-th subsequence --- produces one subsequence converging at every point
of $D$ simultaneously.
