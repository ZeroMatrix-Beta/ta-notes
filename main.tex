...
\AtBeginDocument{%
  ...
  % (no theHainote needed: ainote has its OWN counter, reset per chapter, so \theainote
  %  is already unique document-wide and hyperref's fallback to \theainote is correct.
  %  The entries above exist only because those environments SHARE the theorem counter.)
...
% ainote is the one environment with a counter OF ITS OWN, reset per chapter, rather
% than an \newaliascnt alias of the shared theorem counter.
%
% It was unnumbered until 2026-08-09, so that a \label inside it was silently
% misattributed by cleveref to the last-stepped ambient counter. The obvious repair --
% aliasing it to the theorem counter like everything else -- had already been tried and
% reverted, because 119 AI-Notes stepping the shared counter tore holes in the theorem
% numbering (a Lemma 2.f.43 followed by a Definition 2.f.46, with two AI-Notes in
% between). An independent counter is the repair that has neither problem: AI-Notes can
% carry a \label and be \cref'd as "AI-Note 15.1", and the theorem numbers do not move.
%
% \theainote is \thechapter.\arabic{ainote}, unique document-wide (letters after
% \appendix), so no \theHainote entry is required for hyperref.
%
% What did NOT change is what belongs in one: an AI-Note is editorial commentary about
% the transcription, and the two tests in style.md still decide ainote vs remark vs a
% plain % comment. Being able to reference one is not a reason to file content there.
