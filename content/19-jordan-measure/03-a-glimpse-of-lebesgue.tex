...
% Feedback: Gemini 3.1 Pro (2026-08-11) --- this header, and the opening paragraphs that repeat
% its content to the reader, were called excellent for future maintainers: they are what stops a
% later pass deleting the section as out of scope. Keep them.
...
integral, and yet the term appears on the problem sheets (\cref{ex:jordan_measure_properties_tf}\textbf{(b)}) and in the
...
% Correction: Claude Opus 5 (Ultracode) --- this cited \cref{def:improper_integral} for an
% integral that definition explicitly excludes: it is stated for non-negative $f$ only, and
% rem:improper_needs_nonnegativity says in as many words that $\int_0^\infty (\sin x)/x$ is *not*
% an improper integral in that sense. Two statements in the same chapter therefore contradicted
% each other on the same example. The mathematical point is untouched; it needs the classical
% limit-of-proper-integrals notion, which is what ex:feynman_dirichlet_integral actually uses.
which exists as the limit $\lim_{R\to\infty}\int_0^R$ of proper Riemann integrals (\emph{not}
as an improper integral in the sense of \cref{def:improper_integral}, whose non-negativity
hypothesis this integrand fails, as \cref{rem:improper_needs_nonnegativity} spells out), but
not as a Lebesgue
...
\end{enumerate}
\exsol{sol:ai_lebesgue_null_countable}
\end{aiexercise}
...
\end{enumerate}
\exsol{sol:ai_lebesgue_three_valued}
\end{aiexercise}
...
\end{enumerate}
\exsol{sol:ai_dominated_beats_uniform}
\end{aiexercise}
...
\end{enumerate}
\exsol{sol:ai_improper_not_lebesgue}
\end{aiexercise}
