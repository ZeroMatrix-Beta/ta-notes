...
\end{enumerate}
\exsol{sol:banach_hypotheses_sharp}
\end{exercise}
...
% August 2025, Aufgabe 15 a), printed p. 14
% Extractor: Claude Opus 5 (High)
\begin{exercise}[A fixed point without a complete domain]
\label{ex:kobel_local_banach_fixed_point}
Let $E \subseteq X$ be a non-empty subset of a \emph{complete} metric space $X$, and let
$f : E \to X$ be a contraction, so that
\[d\big(f(x),f(y)\big) \le L\,d(x,y) \qquad \text{for all } x,y \in E, \text{ with } L < 1.\]
Suppose further that there are $x_0 \in E$ and a fixed $r > 0$ such that the \emph{closed} ball
$\overline{B}_r(x_0)$ is contained in $E$, and that
\[d\big(f(x_0),x_0\big) \le (1-L)\,r.\]
Show that $f$ has a fixed point in $E$: there exists $\bar x \in E$ with $f(\bar x) = \bar x$.
\exinfo{This exercise is taken from the Analysis II examination of August 2025
(Prof.\ Laura Kobel-Keller), Problem 15 a).}
\exsol{sol:kobel_local_banach_fixed_point}
\end{exercise}

% Generator: Claude Opus 5 (High)
\begin{remark}[What this buys over \cref{thm:banach_fixed_point}]
\label{rem:local_banach_fixed_point}
Read the hypotheses against the theorem's and the exercise stops looking like a technical variant.
Three things are given up. The domain $E$ is \emph{not} assumed complete, nor even closed. The map
does \emph{not} send $E$ into $E$; its codomain is all of $X$. And $E$ may be far too large for
the iteration to stay inside it.

What replaces them is one inequality. The condition $d(f(x_0),x_0) \le (1-L)r$ says the very first
step is short relative to the room available, and the geometric series then guarantees that the
\emph{whole} orbit never leaves $\overline{B}_r(x_0)$: the total distance travelled is at most
$\sum_{n\ge0}L^n\,d(f(x_0),x_0) \le r$. So the theorem is applied not to $E$ but to
$\overline{B}_r(x_0)$, which is closed in the complete space $X$ and therefore complete, and which
$f$ does map into itself. This is how the fixed-point theorem is used in practice — in the proof
of \cref{thm:inverse_function_theorem}, for instance, one never has a globally invariant complete
set to hand, only a small ball on which the first step is controlled.
\end{remark}
...
\end{tabular}
\end{center}
\exsol{sol:bfpt_applicability_table}
\end{exercise}
...
\label{ex:fixed_points_multiple_choice}
...
\exsol{sol:fixed_points_multiple_choice}
\end{exercise}

% Source: old_exams/WS22_Prüfung.pdf (24 January 2022), Teil B, Aufgabe 2(d), p. 13
% Extractor: Gemini 3.6 Flash (High)
% Correction: Claude Opus 5 (High) --- three repairs. (1) Flash set the problem in "a complete
% metric space (X,d)", where the expression f(x)/lambda that the whole argument turns on is not
% defined: a metric space has no scalar multiplication. The source states it for f : R^n -> R^n,
% restored here. (2) Flash cited "the February 2025 exam, Problem 2 of Part A"; the problem is not
% in that exam at all. (3) Moved here from 01-completeness.tex, which sits before
% thm:banach_fixed_point and so quoted a theorem the reader had not met yet.
\begin{exercise}[Contraction mappings and the equation $f(x) = \lambda x$]
\label{ex:banach_fixed_point_linear}
Let $f : \mathbb{R}^n \to \mathbb{R}^n$ be $L$-Lipschitz with respect to the Euclidean metric, for
some $L > 0$, and let $\lambda > L$.
\begin{enumerate}[label=\textbf{(\alph*)}]
  \item Show that $g : \mathbb{R}^n \to \mathbb{R}^n$, $g(x) := \lambda^{-1} f(x)$, is a contraction.
  \item Conclude that the equation $f(x) = \lambda x$ has a unique solution $x \in \mathbb{R}^n$.
\end{enumerate}
\exinfo{This exercise is taken from the Analysis I \& II exam of 24 January 2022 (Prof.\ Giovanni Felder), Part B, Problem 2\textbf{(d)}. The
original asks only for the conclusion, and only for all sufficiently large $\lambda$; the split into
two parts, and the explicit threshold $\lambda > L$, are added here.}
\exsol{sol:banach_fixed_point_linear}
\end{exercise}

% Source: old_exams/examFS24.pdf (21 August 2024), Wahr/Falsch-Fragen, Aufgabe 2, p. 2
% Extractor: Claude Opus 5 (High)
\begin{exercise}[True or False \textnormal{(the hypotheses of Banach's theorem)}]
\label{ex:banach_hypotheses_true_false}
Let $X := [0,1]$ carry the Euclidean metric, and let $f : X \to X$ be $f(x) := x^2$. Decide whether
each of the following is true or false, with justification.
\begin{enumerate}[label=\textbf{(\alph*)}]
  \item $f$ has Lipschitz constant $2$.
  \item $X$ and $f$ satisfy the hypotheses of the Banach fixed point theorem.
  \item $f$ has fixed points.
\end{enumerate}
\exinfo{This exercise is taken from the exam of 21 August 2024 (Prof.\ Joaquim Serra), where it appears as the second block
of true/false questions.}
\exsol{sol:banach_hypotheses_true_false}
\end{exercise}

% Source: old_exams/examHS24.pdf (13 February 2025), Wahr/Falsch-Fragen, Aufgabe 2, p. 2
% Extractor: Gemini 3.1 Pro (High)
\begin{exercise}[True or False \textnormal{(the hypotheses of Banach's theorem, part II)}]
\label{ex:examHS24_TF_banach}
Let $X := [-\pi,\pi]$ carry the Euclidean metric, and let $f : X \to X$ be $f(x) := \cos(x)$. Decide whether
each of the following is true or false.
\begin{enumerate}[label=\textbf{(\alph*)}]
  \item $f$ has Lipschitz constant $1$.
  \item $X$ and $f$ satisfy the hypotheses of the Banach fixed point theorem.
  \item $f$ has a fixed point.
\end{enumerate}
\exinfo{This exercise is taken from the exam of 13 February 2025 (Prof.\ Joaquim Serra), where it appears as the second
block of true/false questions.}
\exsol{sol:examHS24_TF_banach}
\end{exercise}
