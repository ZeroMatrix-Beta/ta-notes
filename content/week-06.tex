...
\begin{tikzpicture}[>=Latex, scale=1.0]
  \draw[->] (-0.3,0) -- (5.2,0);
  \draw[blue!70!black, thick, domain=0.2:5, samples=50, smooth]
    plot ({\x}, {0.18*(\x-2.6)*(\x-2.6)+0.4});
  \draw[orange!90!black, thick] (0.6,1.34) -- (4.6,0.85);
  \fill (0.6,1.34) circle (1.5pt) node[above, font=\small] {$f(x)$};
  \fill (4.6,0.85) circle (1.5pt) node[above, font=\small] {$f(y)$};
  \fill (2.6,1.06) circle (1.5pt) node[above, font=\scriptsize] {chord};
  \node[below, font=\small] at (0.6,0) {$x$};
  \node[below, font=\small] at (4.6,0) {$y$};
\end{tikzpicture}
\end{center}

\begin{ainote}
Recall from \cref{def:sign_symmetric_matrix} the sign classes of a symmetric matrix
(positive/negative definite, indefinite, degenerate). The proposition below needs the boundary
cases as well: a symmetric $A \in \mathbb{R}^{n\times n}$ is \newterm{positive semidefinite}
(\germanterm{positiv semidefinit}), also called \textbf{nonnegative definite}, if
$v\transp Av \geq 0$ for all $v \in \mathbb{R}^n$; and \newterm{negative semidefinite}
(\germanterm{negativ semidefinit}), or \textbf{nonpositive definite}, if $v\transp Av \leq 0$ for
all $v$. These terms are used freely from \cref{ex:6.8} onward but were not named when
\cref{def:sign_symmetric_matrix} was introduced in Week 5; recorded here instead.
\end{ainote}
...
\begin{tikzpicture}[>=Latex, scale=1.15]
  ...
  % Tangent-ish lines through s and t, matching the slopes of the convex curve there
  \draw[orange!90!black, thick, domain=0.3:4.3]
    plot ({\x}, {-0.4*(\x-1.2) + 1.86});
  \draw[purple, thick, domain=1.7:5.0]
    plot ({\x}, {0.72*(\x-3.6) + 1.39});

  % Labels placed well clear of both lines and the curve, with leader lines to the
  % point they describe (the line's value directly above s, resp. t).
  \node[orange!90!black, font=\scriptsize, align=center] (LabU) at (1.2,2.75) {$Df_u(r(y-x))$\\$+\ \text{const}$};
  \draw[orange!90!black, thin, ->] (LabU) -- (1.2,1.96);
  \node[purple, font=\scriptsize, align=center] (LabV) at (3.6,0.35) {$Df_v(r(y-x))$\\$+\ \text{const}$};
  \draw[purple, thin, ->] (LabV) -- (3.6,1.29);
  ...
  \draw[dashed, gray] (1.2,0) -- (1.2,1.86);
  \draw[dashed, gray] (3.6,0) -- (3.6,1.39);
\end{tikzpicture}
\end{center}
...
\begin{ainote}
Corsin illustrates \textbf{A2} with a second sketch of a wiggly $C^\infty$ curve that is locally
invertible (everywhere invertible differential) but not globally injective: a plateau where it is
"invertible, but not diffeo" locally, a hump where a horizontal level meets the graph twice
("not invertible"), and a final monotone stretch where the inverse function theorem does apply
("diffeo by IFT"). See \texttt{FIG-W06-03}.
\end{ainote}

% Sonnet 5 (Medium) --- FIG-W06-03, drawn from the page image seen while transcribing
\begin{center}
\begin{tikzpicture}[>=Latex, scale=1.05]
  \draw[->] (-0.3,0) -- (8.2,0) node[right, font=\small] {$x$};
  \draw[->] (0,-0.3) -- (0,5.9);

  \draw[blue!80!black, thick] plot[smooth] coordinates {
    (0,0.3) (0.7,0.95) (1.3,1.55) (1.8,1.85) (2.15,1.95) (2.5,2.05)
    (2.9,2.35) (3.6,3.0) (4.4,2.1) (5.2,3.3) (6.0,4.0) (6.8,4.65) (7.6,5.3)};

  % "Invertible, but not diffeo" -- the flat shoulder near x=1.8..2.5
  \draw[red!80!black, dashed, thick] (1.8,1.95) -- (2.5,1.95);
  \node[red!80!black, font=\scriptsize, align=center] (LabShoulder) at (1.0,3.4)
    {invertible,\\ but not diffeo};
  \draw[red!80!black, thin, ->] (LabShoulder) -- (2.1,2.05);

  % "Not invertible" -- the hump between x=3.1 and x=5.0, three crossings at y=2.6
  \draw[red!80!black, dashed, thick] (3.1,2.6) -- (5.0,2.6);
  \node[red!80!black, font=\scriptsize, align=center] (LabHump) at (5.9,1.2)
    {not\\ invertible};
  \draw[red!80!black, thin, ->] (LabHump) -- (4.15,2.55);

  % "Diffeo by IFT" -- the final monotone stretch
  \node[green!60!black, font=\scriptsize, align=center] (LabIFT) at (5.6,4.9)
    {diffeo\\ by IFT};
  \draw[green!60!black, thin, ->] (LabIFT) -- (6.7,4.55);
\end{tikzpicture}
\end{center}
...
In the picture, this means that the \newterm{level set} (\germanterm{Niveaumenge})
$\{f(x,y)=0\}$ can locally be expressed as a graph over $x$.

\begin{remark}[Why $\Jac g$ has this form]
Differentiating the identity $f(x,g(x)) \equiv 0$ (which holds by definition of $g$) with the
chain rule gives
$$0 = D_xf(x,g(x)) + D_yf(x,g(x))\cdot Dg(x),$$
and solving for $Dg(x)$ --- valid since $D_yf(x,g(x))$ is invertible near $(x_0,y_0)$ --- recovers
exactly the formula for $\Jac g(x)$ above.

\begin{ainote}
Corsin remarks in his own notes that ``the geometrical meaning of this one eludes me, sorry''; the
chain-rule derivation above at least pins down where the formula comes from algebraically.
\end{ainote}
\end{remark}

% Sonnet 5 (Medium) --- FIG-W06-04, drawn from the page image seen while transcribing
\begin{center}
\begin{tikzpicture}[>=Latex, scale=1.05]
  \draw[green!60!black, thick, ->] (-0.3,0) -- (3.6,0) node[right, font=\small] {$x \in \mathbb{R}^d$};
  \draw[orange!90!black, thick, ->] (0,-0.3) -- (0,3.3) node[above, font=\small] {$y \in \mathbb{R}^{n-d}$};

  \coordinate (P) at (1.6,1.9);
  \draw[dotted, gray] (1.6,0) -- (P) -- (0,1.9);
  \node[below, font=\small] at (1.6,-0.05) {$x_0$};
  \node[left, font=\small] at (-0.05,1.9) {$y_0$};
  \fill (P) circle (1.5pt) node[above right, font=\small] {$(x_0,y_0)$};
  \draw[orange!90!black, thick, ->] (P) -- ++(0,1.0) node[above, font=\scriptsize] {$\partial y$};

  \draw[purple, thick, ->] (2.0,2.4) .. controls (3.2,3.0) .. (4.6,2.0)
    node[midway, above, font=\scriptsize] {$D_yf(x_0,y_0)$};

  \draw[->, thick] (4.9,0) -- (4.9,3.2) node[above, font=\small] {$\mathbb{R}^{n-d}$};
  \fill (4.9,2.0) circle (1.5pt) node[right, font=\small] {$0=f(x_0,y_0)$};
  \draw[->, thick, blue!80!black] (P) .. controls (3.4,0.6) .. (4.9,2.0)
    node[midway, below, font=\small] {$f$};
\end{tikzpicture}
\end{center}

\begin{center}
\begin{tikzpicture}[>=Latex, scale=1.05]
  \draw[green!60!black, thick, ->] (-0.3,0) -- (4.3,0) node[right, font=\small] {$x \in \mathbb{R}^d$};
  \draw[orange!90!black, thick, ->] (0,-0.3) -- (0,3.3) node[above, font=\small] {$y \in \mathbb{R}^{n-d}$};

  \draw[green!60!black, thick] (0.6,-0.08) -- (0.6,0.08) node[below, font=\scriptsize] {\phantom{x}};
  \draw[green!60!black, thick] (0.6,-0.08) node[below, font=\scriptsize] {$x_0$};
  \draw[green!60!black] (0.4,0) rectangle (3.4,1.9);
  \node[green!60!black, font=\scriptsize] at (1.9,-0.3) {$B_r(x_0)$};
  \draw[orange!90!black] (0.4,0.5) -- (0.35,0.5) node[left, font=\scriptsize] {$B_s(y_0)$};

  \draw[purple, thick, domain=0.4:3.4, samples=60, smooth]
    plot ({\x}, {1.1 + 0.35*sin((\x-0.4) r * 2.2)}) node[midway] {};
  \node[purple, font=\scriptsize, above] at (1.9,1.55) {$(x,g(x))$};

  \draw[->, thick] (5.1,0) -- (5.1,3.2) node[above, font=\small] {$\mathbb{R}^{n-d}$};
  \fill (5.1,1.5) circle (1.5pt) node[right, font=\small] {$0=f(x,g(x))$};
  \draw[->, thick, blue!80!black] (1.9,1.55) .. controls (3.8,2.4) .. (5.1,1.5)
...
\begin{ainote}
At $\bar x = 1$ (where $y=0$), $D_yF(1,0)=0$ fails to be bijective, and indeed no such graph
$y(x)$ exists around $y=0$ there --- the circle is vertical at that point, so it cannot locally be
written as a graph over $x$.
\end{ainote}
\end{example}
