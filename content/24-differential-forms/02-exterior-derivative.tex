...
explicitly in coordinates and properties (1)--(3) are verified.
\end{ainote}
...
\begin{enumerate}[label=\textbf{\arabic*.}]
  ...
  completes the pattern of items 1 and 2 above: $\grad$, $\curl$ and $\divg$ are exactly the
...
% curl and div are one operator satisfying one identity. Items 1-3 of the example above prove
...
computations just performed: the left square is item 1, the middle is item 2, the right is
item 3. Read downward it says that the classical vector operators are $d$ in disguise, and that
the disguise is nothing but the choice of which object to call a vector field.

The payoff is the bottom row's two mysteries collapsing into one line of the top row. Property
(3) of \cref{thm:exterior_derivative} is $d \circ d = 0$, a single statement about a single
...
\begin{enumerate}[label=\textbf{\arabic*.}]
  ...
  \item By part 1 and \cref{thm:exterior_derivative}, one might hope that $\omega = df$ for some
