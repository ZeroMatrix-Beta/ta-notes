...
\begin{enumerate}[label=\textbf{\arabic*.}]
  ...
  \item \textit{Continuity.} On $\mathbb{R}\setminus\{0\}$, $g = h/t$ is a quotient of smooth
    ...
    part 1 is precisely the limit, so $g$ --- and hence $f = e^{g}$ --- is continuous everywhere.
  ...
  \item \textbf{(*) Yes} --- and considerably more than $C^1$. Each $x_i^{\,t} = e^{t\log x_i}$ is
    ...
    $f = e^{g}$, is real-analytic on all of $\mathbb{R}$ --- in particular $C^\infty$, so
    certainly $C^1$.
    ...
    $f'(0) \geq 0$, with equality exactly when all $x_i$ agree --- in which case $\mu_t(x)$ is
    that common value for every $t$ and $f$ is constant.
\end{enumerate}
\end{exercisesolution}

\begin{ainote}
Jensen's inequality, used above without being stated, is now available as
\cref{prop:jensen_inequality} (\cref{ch:convexity}). The official hint for 4.4.2 routes through the concavity of $\log$ together with
Cauchy--Schwarz. The solution above instead applies Jensen once to $\varphi(s) = s\log s$, which
is convex; the two arrive at the same inequality, and the second makes visible why 4.4.2 and
4.4.3 are the same statement --- substituting $a_i := x_i^{\,t}$ turns the inequality of part 2
into the sign of $g'(t)$ in part 3. That is worth seeing, because the monotonicity of the power
means is the whole point of the problem and the classical AM--GM inequality falls out of it as
the single comparison $\mu_0 \leq \mu_1$.
\end{ainote}
