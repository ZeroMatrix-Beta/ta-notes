...
\begin{itemize}
  ...
  \item \textbf{A scalar field} ($n$ arbitrary, $m=1$): one row, so $\Jac f(x)$ is a
    \emph{row vector} --- the transposed gradient, $(\nabla f)\transp$.
...
\end{itemize}
The gradient being a row and the velocity a column is not a convention to memorise: it follows
from \qt{$m$ rows, $n$ columns} and nothing else. Confusing the two is the commonest source of
shape errors when the chain rule is applied later in this chapter.
...
\begin{tikzpicture}[>=Latex, node distance=0pt]
  ...
  \node[font=\scriptsize, red!75!black, align=right, anchor=east] at (-2.6,3.7)
    {\textbf{no:} \cref{ex:differentiable_not_c1}};
...
\begin{itemize}
  ...
  \item the \textbf{quadratic} term $\tfrac12 x\transp\Hess f(x_0)\,x$ --- the first term that
    sees curvature, and the one that decides in Week 5 whether a critical point is a minimum, a
    maximum, or a saddle.
\end{itemize}
...
\begin{enumerate}[label=\textbf{\arabic*.}]
  \item For $n=2$, $\{x : |x|_1 \leq 1\}$ is the diamond with vertices $(\pm 1, 0)$, $(0,\pm 1)$;
    ...
    the four points $(\pm1,0)$, $(0,\pm1)$ --- since $|e_i|_p = 1$ for every $p$ --- and they
    increase with $p$, which is the geometric content of part 2.
    \begin{center}
