...
% Correction: Claude Opus 5 (Ultracode) --- the statement wrote $\mu_n(A_t)$ without ever
% assuming the slices are Jordan measurable, which does not follow from Jordan measurability of
% $A$: see the ainote below for a set whose slice at one height is $\mathbb{Q}\cap[0,1]$. The
% hypothesis is now stated. (Every set the document applies this to has interval or ball slices,
% so no application changes.)
Let $A, B \subseteq \mathbb{R}^{n+1}$ be Jordan measurable sets. For each height $t \in \mathbb{R}$, denote by $A_t := \{x \in \mathbb{R}^n \mid (x,t) \in A\}$ and $B_t := \{x \in \mathbb{R}^n \mid (x,t) \in B\}$ the $n$-dimensional cross-sections at $t$, and suppose every one of these slices is itself Jordan measurable. If the cross-sectional volumes agree for all $t \in \mathbb{R}$, i.e.\
\[
...
\begin{proof}
Both sets are bounded, so fix a box $Q\times[c,d]$ containing them. Integrating the indicator
function over slices gives:
\[
...
\begin{ainote}
\label{note:cavalieri_hypotheses}
% Correction: Claude Opus 5 (Ultracode) --- the proof cited thm:fubini, which is stated for a
% *continuous* integrand on a product of boxes, for an indicator function on all of R^{n+1}. The
% step is standard but it is not the theorem cited, so the citation is replaced by this note
% rather than left pointing at a statement that does not cover it.
Two things in that proof are being used in slightly stronger form than
\cref{thm:fubini} states them, and both are worth naming.

First, $\chi_A$ is not continuous, so the version of Fubini in play is the one for
\emph{Riemann integrable} functions on a box: $\chi_A$ is Riemann integrable there precisely
because $A$ is Jordan measurable, and the iterated identity holds provided the inner integral
exists at every $t$, which is what the slice hypothesis buys.

Second, that hypothesis is not free. Take
\[A := \big([0,1]\times(0,1]\big) \ \cup\ \big((\mathbb{Q}\cap[0,1])\times\{0\}\big)
  \ \subseteq\ \mathbb{R}^2 .\]
Its interior is $(0,1)^2$ and its closure is $[0,1]^2$, so $\partial A$ is the frame of the unit
square, which is $\vol_2$-null: $A$ is Jordan measurable with $\mu_2(A) = 1$. But
$A_0 = \mathbb{Q}\cap[0,1]$ is not Jordan measurable, and $\mu_1(A_0)$ simply does not exist. A
single bad slice is enough to make the inner integral undefined, which is why the hypothesis is
stated on every $t$ rather than derived.
\end{ainote}
...
\begin{tikzpicture}[>=Latex, scale=1.0]
  ...
  % Correction: Claude Opus 5 (Max), found by rendering p.286 --- centred at (0.75,1.05) this
  % 0.75-unit box ran from x = 0.375, and the cone's left edge passes x = 0.333 at that height
  % and x = 0.30 just above it: the blue side printed through "$(1-$". There is no room to
  % centre it higher either, since the cone narrows -- at y = 1.05 the whole section is only
  % 0.82 units across. Set outside the cone, past the right-hand end of the section it names
  % (the cone's right edge is at x = 1.069 at the box's floor).
  \node[green!50!black, font=\scriptsize, anchor=south west] at (1.20, 1.04) {$(1-t)\Omega$};

  % Correction: Claude Opus 5 (Max) --- both of these were struck through by a line. `$t$' sat
  % `left' at (0,1.0), centred on y = 1.0, and the dashed cross-section guide runs (-0.3,1.0) to
  % (2.8,1.0) straight through it; `$0$' sat `left' at (0,0) with the $x$ axis running from
  % (-0.5,0) through it. Same fault and same repair as the struck-through zero in the convexity
  % figure of ch. 14: set each off the corner, where both lines are clear of it.
  \node[font=\scriptsize, anchor=south east] at (-0.10, 1.04) {$t$};
  \node[font=\scriptsize, anchor=north east] at (-0.10, -0.06) {$0$};
...
\begin{enumerate}[label=\textbf{(\alph*)}]
  ...
  \item Evaluate $I$.
\end{enumerate}
\exsol{sol:ai_order_swap_gaussian}
\end{aiexercise}
...
\exsol{sol:gaussian_slab_alpha_asymptotics}
\end{exercise}
