...
% Generator: Claude Opus 5 (High) --- the one-variable arc-length formula, recovered as the
% $d=1$ case of the Gram determinant, so that the familiar formula is visibly a special case
...
\[\sqrt{\det\big(\Jac\phi(t)\transp\Jac\phi(t)\big)} = \lVert\phi'(t)\rVert
  = \sqrt{1+f'(t)^2},\]
...
\[\lVert\gamma'(t)\rVert = 3\cos t\,\sin t\sqrt{\cos^2t+\sin^2t} = 3\cos t\,\sin t
  = \tfrac32\sin(2t).\]
