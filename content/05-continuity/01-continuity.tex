...
\begin{tikzpicture}[>=Latex, scale=1.0]
  ...
  % Correction: Claude Opus 5 (Max) --- anchored north west at (4.35,-0.35), this label occupied
  % roughly x = 4.35 to 5.65 at heights -0.35 to -0.65, and the $\varepsilon$-ball (centre
  % (3.95,0.05), radius 0.78) passes x = 4.62 at y = -0.35 and x = 4.35 at y = -0.62: the red
  % circle cut a diagonal straight across the label's opening glyphs. Nowhere inside the ball
  % takes it -- a 1.3cm box does not fit between the green blob's edge and the arc anywhere --
  % so it goes below the ball, where the gap down to $Y$'s own lower edge (about y = -1.45 at
  % this abscissa) is clear, with a leader back to the region it names.
  \draw[OliveGreen, thin] (3.95,-0.87) -- (3.95,-0.50);
  \node[OliveGreen, font=\scriptsize, anchor=north] at (3.95,-0.85)
...
\]
Since these limits disagree, $\lim_{(x,y) \to (0,0)} f(x,y)$ does not exist. A useful method for
checking limits at the origin is to switch to \newterm{polar coordinates}
($x = r\cos\varphi, y = r\sin\varphi$). What must be produced is a bound
% Correction: Claude Opus 5 (Ultracode) --- this sentence read "If the limit as $r \to 0$ is
% independent of $\varphi$, then the function converges", which is false; see the ainote below.
% The solution of ex:problems_at_origin always argued correctly ("the bound is uniform in
% $\theta$"), so only this statement of the method was wrong, not any use of it.
\[\big\lvert f(r\cos\varphi,\,r\sin\varphi) - L\big\rvert \ \leq\ g(r)
  \qquad\text{with } g(r) \to 0 \text{ as } r \downarrow 0,\]
where $g$ does \emph{not} depend on $\varphi$; then $\lim_{(x,y)\to(0,0)}f(x,y) = L$.
(This is explored further in \cref{ex:problems_at_origin}).
\end{example}

\begin{ainote}
\label{note:polar_limits_need_uniformity}
The uniformity is the whole content of the method, and dropping it is the standard trap. It is
\emph{not} enough that $\lim_{r\to0}f(r\cos\varphi,r\sin\varphi)$ exist for each fixed $\varphi$
and take the same value throughout: that only says the limit is $L$ along every straight ray,
and a function can approach the origin along a curve without ever following a ray. For
\[f(x,y) := \frac{x^2y}{x^4+y^2}, \qquad
  f(r\cos\varphi,r\sin\varphi) = \frac{r\cos^2\varphi\sin\varphi}{r^2\cos^4\varphi+\sin^2\varphi},\]
the limit as $r \downarrow 0$ is $0$ for every fixed $\varphi$, including $\sin\varphi = 0$,
where $f$ vanishes identically. Yet along the parabola $y = x^2$ one gets
$f = x^4/(2x^4) = \tfrac12$, so the limit does not exist. The denominator is what defeats the
ray test: it is small precisely where $\sin\varphi$ is comparable to $r$, which no fixed
$\varphi$ ever sees.
\end{ainote}
...
\label{ex:continuity_of_distance}
...
\label{ex:continuity_of_composition}
...
\exsol{sol:continuity_of_composition}
\end{exercise}

\begin{remark}
The three equivalent definitions of continuity referenced in \cref{ex:continuity_of_composition} are exactly the ones
...
$f_E(x) := d(x,E) = \inf_{z\in E}d(x,z)$ is $1$-Lipschitz, which is \cref{ex:distance_from_set_multiple_choice}\textbf{(c)},
...
\begin{remark}
This sharpens \cref{ex:distance_from_set_multiple_choice}\textbf{(a)}, which showed only one direction ($E$ closed
