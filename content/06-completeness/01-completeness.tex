...
complete space $(\hat X,\hat d)$ containing an isometric copy of $X$ as a dense subset --- the
\newterm{completion} of $(X,d)$, of which $\mathbb{R}$ as the completion of $\mathbb{Q}$ is the
motivating case. The script itself marks this material \qt{extra; cf.\ Grundstrukturen}, and
Corsin's notes never construct a completion, so this document deliberately gives it no section
either (see \texttt{script-coverage.md}, \S6). Consequently script Exercises 9.33--9.35, which
walk through the construction, are not taken here even though they are otherwise easy and would
fit this chapter --- they depend on machinery ($\S9.1.4$) this document does not develop. Anyone
wanting the construction should consult the sibling \emph{Grundstrukturen} project, which is
