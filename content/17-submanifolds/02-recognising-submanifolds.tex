...
\begin{ainote}
\cref{rem:three_submanifold_descriptions} lists the three descriptions and says which to reach
for, but Corsin's notes never run a single surface through all three, and they give no worked
example of a set that \emph{fails} to be a submanifold beyond the crossing in
\cref{ex:cross_not_submanifold}. Linus devotes most of one session to exactly these two gaps, and
this section follows his treatment.
\end{ainote}
...
\begin{itemize}
  \item \textbf{A parametrization need not be global.} The circle $\mathbb{S}^1$ is a
    ...
    (\cref{thm:continuity_preserves_connected}), so no such $\gamma$ exists --- the same counting
    argument as in \cref{ex:cross_not_submanifold}, run in the opposite direction. Two overlapping
...
\end{itemize}
Requiring the descriptions only near each point costs nothing --- being a submanifold is itself a
local condition --- and buys all the examples that matter.
\end{remark}
...
submanifold means looking, near every one of its points, like a flat piece of $\mathbb{R}^d$ ---
and there are several distinct ways to violate that.

\begin{itemize}
  \item \textbf{The figure eight} $\{(x,y) : y^2 = x^2 - x^4\}$ --- a \emph{crossing}. Away from
  ...
  \item \textbf{The lollipop} --- a circle with a segment attached, or any \qt{line sticking out
  ...
  \item \textbf{The closed square} $[0,1]^2 \subseteq \mathbb{R}^2$ --- a \emph{boundary}. Its
  ...
  \item \textbf{$\mathbb{Q}^n \subseteq \mathbb{R}^n$} --- \emph{no dimension at all}. It is not
