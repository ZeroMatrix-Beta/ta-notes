...
% Sascha Brack's notes (Lecture 8) state the sufficient condition as his Theorem 2.9 and organise
% the picture into a chain of implications; both are reproduced below.
\begin{ainote}
The course gives the definition of differentiability and the warnings that go with it, but never
states a \emph{sufficient} condition, so up to this point we have two counterexamples and no
positive test at all. The theorem below supplies one, from a second tutor's notes. Without it the
practical question \qt{how do I actually check differentiability?} has no answer.
\end{ainote}
...
and differentiability follows. That is exactly why the course can write $f \in C^1$, $C^2$ and
...
\begin{tikzpicture}[>=Latex, node distance=0pt]
  ...
  % Opus 5 (High): continuity hangs off the main chain to the right rather than sitting in it,
  % because it is not a link --- differentiability implies it, but it implies nothing below.
  % Placed level with (df) so the green arrow is horizontal and clears both green labels,
  % which sit at x = 0.15 between the boxes.
  \node[box, fill=blue!8] (cont) at (5.9,2.9) {$F$ continuous at $x_0$};

  \draw[imp] (c1) -- (df);
  \draw[imp] (df) -- (dir);
  \draw[imp] (dir) -- (par);
  \draw[imp] (df.east) -- (cont.west);
  ...
  \node[font=\scriptsize, green!45!black, above] at (4.0,2.98)
    {\cref{prop:differentiable_implies_continuous}};
  ...
  % The one reverse arrow that does not run up the left-hand chain: it crosses the empty
  % region between (dir) and (cont), so it carries its label inline like the others. Same
  % counterexample as the arrow above -- x^2y/(x^4+y^2) defeats both implications at once,
  % which is exactly why it is the example worth remembering.
  \draw[nimp] (dir.east) -- node[noLbl, align=center]
    {\textbf{no:} \cref{ex:all_directional_derivatives_exist}} (cont.south);
\end{tikzpicture}
\end{center}

Every downward arrow holds, and \textbf{not one of them reverses}. Each failure is witnessed by an
example already in this chapter, so the diagram doubles as a map of where those counterexamples
belong:
\begin{itemize}
  ...
  \item \textbf{All directional derivatives $\not\Rightarrow$ differentiable:}
    ...
    $v \mapsto \partial_vf(0,0)$ is not linear. The same example defeats the branch to the
    right as well: it is not even \emph{continuous} at the origin, so having every directional
    derivative buys strictly less than continuity does.
  \item \textbf{Partial derivatives $\not\Rightarrow$ all directional derivatives:}
    ...
    so $\partial_vf(0,0)$ exists only along the two axes, which are precisely the two directions
    the partial derivatives happen to look at.
\end{itemize}
...
\begin{enumerate}[label=\textbf{(\alph*)}]
  \item \textbf{Compute the partial derivatives $\partial_iF(x_0)$.} If one of them fails to
    ...
    \Cref{thm:continuous_partials_imply_differentiable}, and you are done again. Most exercises
    end here.
  ...
  \item \textbf{Verify the candidate against the definition:}
    \[\lim_{h\to0}\frac{\big|F(x_0+h)-F(x_0)-L(h)\big|}{|h|} \overset{?}{=} 0.\]
    In $\mathbb{R}^2$, substituting $h = (r\cos\theta,\ r\sin\theta)$ and checking that the
    result tends to $0$ \emph{uniformly in $\theta$} is usually the quickest route. This is the
    same polar trick used in \cref{ex:all_directional_derivatives_exist} and, later, in
    \Cref{q:quiz_q4}.
\end{enumerate}
...
\begin{enumerate}[label=\textbf{(\alph*)}]
  ...
  \item \textbf{Verify against the definition:} We check whether the following limit is zero:
    \[\lim_{h\to 0} \frac{|f(h_1,h_2) - f(0,0) - L(h)|}{|h|} = \lim_{h\to 0} \frac{\frac{h_1^2 h_2}{h_1^2+h_2^2}}{\sqrt{h_1^2+h_2^2}} = \lim_{h\to 0} \frac{h_1^2 h_2}{(h_1^2+h_2^2)^{3/2}}.\]
...
\begin{enumerate}[label=\textbf{(\alph*)}]
  ...
  \item \textbf{Verify against the definition:} We check the limit
    \[\lim_{h\to 0} \frac{|f(h_1,h_2) - f(0,0) - L(h)|}{|h|} = \lim_{h\to 0} \frac{\frac{h_1^2 h_2^2}{\sqrt{h_1^2+h_2^2}}}{\sqrt{h_1^2+h_2^2}} = \lim_{h\to 0} \frac{h_1^2 h_2^2}{h_1^2+h_2^2}.\]
...
% Source: old_exams/examFS24.pdf (21 August 2024), Wahr/Falsch-Fragen, Aufgabe 4, p. 3
% Extractor: Claude Opus 5 (High)
% The function is ex:differentiability_recipe_application with x and y interchanged, so the origin
% computation is one the reader has already seen. Kept anyway, and kept as an exercise rather than
% a second worked example: what the exam adds is the full regularity ladder (smooth off the origin,
% one partial derivative, continuity, C^1) rather than the single differentiability verdict, and
% part (c) against part (d) is the pair the whole block is built around.
\begin{exercise}[True or False \textnormal{(how much regularity does this function have?)}]
\label{ex:regularity_ladder_xy2}
...
\begin{enumerate}[label=\textbf{(\alph*)}]
  \item $f \in C^\infty\big(\mathbb{R}^2\setminus\{(0,0)\}\big)$.
  \item $\partial_x f(0,0)$ exists.
  \item $f \in C^0(\mathbb{R}^2)$.
  \item $f \in C^1(\mathbb{R}^2)$.
\end{enumerate}
\exinfo{This exercise is taken from the exam of 21 August 2024 (Prof.\ Joaquim Serra), where it appears as the fourth
block of true/false questions.}
\exsol{sol:regularity_ladder_xy2}
\end{exercise}
