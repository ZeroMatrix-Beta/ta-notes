...
\begin{ainote}
% Correction: Claude Opus 5 (Ultracode) --- this inventory named only two of the sites needing
% the piecewise version, and the document has five. The three it missed were
% sol:vector_field_flux_hemisphere (which additionally *asserted* the upper half ball was a
% bounded C^1 domain -- corrected there), sol:ai_capping_a_paraboloid, and ex:divergence_cylinder.
% An inventory that is silently incomplete is worse than none, since the next reader takes its
...
piecewise smooth domain without proof. It is worth having written down, because this document was
already using it in five places. A cube has edges and corners and is not a $C^1$ domain, so
...
where the paraboloid meets its lid. The same goes for \cref{ex:divergence_cylinder}, a solid
cylinder with two edge circles; for \cref{sol:vector_field_flux_hemisphere}, an upper half ball
whose hemisphere meets its equatorial disc at a right angle; and for
\cref{sol:ai_capping_a_paraboloid}, capped on the rim $x^2+y^2=1$.
...
We compute the outward flux of $F$ through $\partial\Omega$ using the divergence theorem in
the piecewise form \cref{thm:gauss_divergence_piecewise}, needed here because the two rim circles
where the lateral surface meets the lids are edges, so $\Omega$ is not a $C^1$ domain.
...
\label{ex:radial_vector_fields_flux}
...
\exsol{sol:radial_vector_fields_flux}
\end{exercise}
...
\end{enumerate}
\exsol{sol:ai_flux_cubic_sphere}
\end{aiexercise}
...
\begin{enumerate}[label=\textbf{(\alph*)}]
  ...
  \item Compute the flux through $D$ alone, directly from \cref{def:flux}, and deduce the flux
    of $F$ through $S$.
\end{enumerate}
\exsol{sol:ai_capping_a_paraboloid}
\end{aiexercise}
...
\exsol{sol:examHS24_KP1_part2_flux}
\end{exercise}
...
\exsol{sol:gauss_law_ellipsoid_flux}
\end{exercise}
...
\exsol{sol:tuning_alpha_prescribed_flux}
\end{exercise}
...
\exsol{sol:flux_truncated_paraboloid}
\end{exercise}
...
\exsol{sol:radial_weight_flux_independent}
\end{exercise}
...
\exsol{sol:knoerrer_divergence_free_compact_support}
\end{exercise}
...
\exsol{sol:green_first_identity_harmonic}
\end{exercise}

% Source: old_exams/fs2023/examsol.pdf (Analysis II, 18 August 2023, Prof. Urs Lang), Aufgabe 3.A1 (c)
% Extractor: Gemini 3.7 Flash (High)
% The mean value property and the maximum principle for harmonic functions, proved directly
% from Gauss's divergence theorem. Pairs with ex:green_first_identity_harmonic: there uniqueness
% came from Green's first identity, here it comes from the spherical mean value property.
\begin{exercise}[The mean value property and maximum principle for harmonic functions]
\label{ex:harmonic_mean_value_maximum_principle}
Let $\Omega \subseteq \mathbb{R}^n$ ($n \geq 2$) be an open connected set, and let $u \in C^2(\Omega)$
be harmonic, so that $\Delta u = \divg(\nabla u) = 0$.
For a point $x_0 \in \Omega$ and radius $R > 0$ such that $\overline{B_R(x_0)} \subset \Omega$, define
for $r \in (0, R]$ the spherical average
\[
\phi(r) := \frac{1}{\omega_{n-1} r^{n-1}} \int_{\partial B_r(x_0)} u(x) \, d\vol_{n-1}(x),
\]
where $\omega_{n-1} r^{n-1} = \vol_{n-1}(\partial B_r(x_0))$ is the surface area of the $(n{-}1)$-sphere.
\begin{enumerate}[label=\textbf{(\alph*)}]
  \item By writing $\phi(r) = \frac{1}{\omega_{n-1}} \int_{\partial B_1(0)} u(x_0 + r\xi) \, d\vol_{n-1}(\xi)$
    and differentiating with respect to $r$, apply Gauss's divergence theorem on $B_r(x_0)$ to show that
    $\phi'(r) = 0$ for all $r \in (0, R]$.
  \item Conclude that $\phi(r) = u(x_0)$ for all $r \in (0, R]$ (the spherical \newterm{mean value property}),
    and deduce the solid ball mean value property:
    \[
    u(x_0) = \frac{1}{\alpha_n r^n} \int_{B_r(x_0)} u(x) \, d\vol_n(x),
    \]
    where $\alpha_n r^n = \vol_n(B_r(x_0))$.
  \item Prove the \newterm{strong maximum principle}: if $u$ attains a local minimum (or a local maximum) at some
    point $x_0 \in \Omega$, then $u$ is constant on $\Omega$.
\end{enumerate}
\exinfo{This exercise is taken from the examination of 18 August 2023 (Prof.\ Urs Lang), Problem 3.A1 (c).}
\exsol{sol:harmonic_mean_value_maximum_principle}
\end{exercise}
...
\exsol{sol:flux_through_nested_spheres}
\end{exercise}
