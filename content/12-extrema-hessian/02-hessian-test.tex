...
($\det A = 0$), which is also what \cref{ex:signature_2x2_matrix} states. Corrected above.
\end{ainote}
...
instead, since that is the one students actually run into (\cref{ex:semidefinite_hessian_multiple_choice} is exactly this trap).
...
\exsol{sol:signature_2x2_matrix}
\end{exercise}
...
\begin{exercise}[Multiple choice (what a positive semidefinite Hessian does not give you) \textnormal{(important)}]
\label{ex:semidefinite_hessian_multiple_choice}
...
\exsol{sol:semidefinite_hessian_multiple_choice}
\end{exercise}
...
% Correction: Claude Opus 5 (High) --- the solution is split into two blocks with an ainote
% between them, and only the first was linked, so part (b) had an anchor nothing pointed at.
% One link per part; \cpageref takes a single label, so this cannot be one \exsol with a list.
\exsol[Solution to \textbf{(a)}]{sol:extrema_circle_disk}
\exsol[Solution to \textbf{(b)}]{sol:extrema_circle_disk_interior}
\end{exercise}
...
\label{ex:critical_points_parameterized_gaussian}
...
\exsol{sol:critical_points_parameterized_gaussian}
\end{exercise}
...
\exsol{sol:hessian_parameterized_exp}
\end{exercise}
