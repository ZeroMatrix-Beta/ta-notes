...
% Correction: Claude Opus 5 (High) --- a second anchor, \label{ex:solution_9.1}, stood here:
% numeric, and prefixed ex: on an exercisesolution. It duplicated the sol: label above, which
% the one \cpageref that used it now points at instead.
% Generator: Claude Sonnet 5 (Medium)
Following the official hint rather than Corsin's
(\cref{note:corsin_hint_spherical_not_cylindrical}), we use
...
% Correction: Claude Opus 5 (Ultracode) --- "this is the Gram determinant" names the square root
% of it, which under this chapter's convention is the area element, not the determinant. Same
% slip as in 01-area-and-the-gram-determinant.tex and 03-length-of-a-curve.tex.
$\theta\in(-\pi/2,\pi/2)$, so $\cos\theta>0$). By \cref{prop:flux_r3}\textbf{(b)}, this is the
square root of the Gram determinant, so $dA = \cos\theta\,d\theta\,d\varphi$, and
...
\begin{enumerate}[label=\textbf{(\alph*)}]
  ...
  \item Using the suggested parametrization $\Phi(x,\theta) = (x,f(x)\cos\theta,
    ...
    \end{align*}
    % Correction: Claude Opus 5 (Ultracode) --- as above, this names the square root of the Gram
    % determinant, which is the area element rather than the determinant itself.
    By \cref{prop:flux_r3}\textbf{(b)}, this is the square root of the Gram determinant, so
  ...
  \item ($\Leftarrow$) Suppose $(*)$ holds. Since $f'(a^+)=+\infty$, the inverse
    ...
    $f(a)>0$ to keep the angular direction non-degenerate).

    % Correction: Claude Opus 5 (Ultracode) --- both directions of (e) were proved only for
    % $f(a)>0$, which $(*)$ does not require, and the archetypal instance violates it: the ball
    % $f(x)=\sqrt{1-x^2}$ on $(-1,1)$ satisfies $(*)$ and has $f(a)=f(b)=0$. With $f(a)=0$ the
    % point $P$ used below collapses onto the axis and the curve $(a,f(a)-t,0)$ leaves
    % $\partial\Omega$ entirely. The claim is true in that case too; the missing arguments are
    % supplied here, one per direction.
    If instead $f(a)=0$, that parametrization degenerates and the boundary point to examine is
    $P := (a,0,0)$, on the axis. Since $f'(a^+)=+\infty$, $f$ is increasing near $a$, so its
    inverse $h := f^{-1}$ is defined on $(0,\delta)$ with $h(0^+) = a$ and
    $h'(y) = 1/f'(h(y)) \to 0$ as $y \to 0^+$. Setting $h(0) := a$, the map
    $(y,z) \mapsto h\big(\sqrt{y^2+z^2}\big)$ has partial derivatives $h'(r)\,y/r$ and
    $h'(r)\,z/r$ with $r := \sqrt{y^2+z^2}$, which are continuous and tend to $0$ as $r \to 0$.
    So it is $C^1$ near the origin and exhibits $\partial\Omega$ near $P$ as a $C^1$ graph over
    the $(y,z)$-plane. The symmetric argument at $b$, using
    ...
    Therefore $f>0$ on $(a,b)$. For the endpoint condition at $a$, suppose first that $f(a)>0$.
    Then the point
    ...
    If $f(a)=0$ the relevant boundary point is $P := (a,0,0)$ and the argument is shorter.
    Suppose $f'(a^+) = L$ were finite. Rotation about the $x$-axis maps $\partial\Omega$ to
    itself and fixes $P$, so it acts on $T_P(\partial\Omega)$; the only $2$-plane in
    $\mathbb{R}^3$ invariant under all of those rotations is
    $\operatorname{span}\{e_2,e_3\}$, so that is what $T_P(\partial\Omega)$ must be. But the
    curve $s\mapsto\big(a+s,\ f(a+s),\ 0\big)$ lies in $\partial\Omega$, passes through $P$ at
    $s=0$ and has tangent $(1,L,0)$ there, whose $e_1$-component is $1$. That vector is not in
    $\operatorname{span}\{e_2,e_3\}$, a contradiction, so $f'(a^+)=+\infty$ in this case too.
    Geometrically, a finite slope leaves the body with a conical vertex, or with a cusp when
    $L=0$. The symmetric argument at $b$ gives $f'(b^-)=-\infty$.
\end{enumerate}
\end{exercisesolution}
...
\begin{exercisesolution}[Solution to \cref{ex:ai_circle_two_atlases}]
\label{sol:ai_circle_two_atlases}
% Generator: Claude Opus 5 (Ultracode)
Throughout, $\Jac\Phi$ is the $2\times1$ matrix $\Phi'$, so
$\Jac\Phi\transp\Jac\Phi = \lvert\Phi'\rvert^2$ is a $1\times1$ Gram determinant and its square
root is the familiar speed $\lvert\Phi'\rvert$ of \cref{sec:length_of_curve}.
\begin{enumerate}[label=\textbf{(\alph*)}]
  \item Here $\Phi'(t) = (-\sin t,\cos t)$ has $\lvert\Phi'\rvert \equiv 1$, so
    \[\int_{S^1} 1 \mathop{d\text{vol}}_1 = \int_0^{2\pi} 1\,dt = 2\pi .\]
    The omitted point $(1,0)$ is a single point, hence $\vol_1$-null, and contributes nothing ---
    which is the shortcut \cref{rem:one_chart_recovers_the_earlier_definition} describes.
  \item Let $\eta_1,\eta_2$ be \emph{any} partition of unity subordinate to this atlas, so that
    $\eta_1+\eta_2 \equiv 1$ on $S^1$. Since $\lvert\Phi_i'\rvert \equiv 1$ for both,
    \[\int_{S^1} 1 \mathop{d\text{vol}}_1
      = \int_0^{2\pi}\eta_1(\cos t,\sin t)\,dt + \int_{-\pi}^{\pi}\eta_2(\cos t,\sin t)\,dt .\]
    The second integrand is a $2\pi$-periodic function of $t$, and the integral of such a function
    over any interval of length $2\pi$ is the same, so the second integral may be taken over
    $(0,2\pi)$ as well. Adding the two under one integral sign,
    \[\int_{S^1} 1 \mathop{d\text{vol}}_1
      = \int_0^{2\pi}\big(\eta_1+\eta_2\big)(\cos t,\sin t)\,dt
      = \int_0^{2\pi} 1\,dt = 2\pi ,\]
    and no particular partition was ever chosen. That is
    \cref{prop:integral_over_submanifold_well_defined} in miniature: the $\eta_l$ disappear
    because they sum to $1$, and everything else was already parametrization-independent.
  \item \textbf{The two images miss $(\pm1,0)$.} $\operatorname{Im}(\Psi_+)$ is the open upper
    semicircle and $\operatorname{Im}(\Psi_-)$ the open lower one, so their union is
    $S^1\setminus\{(1,0),(-1,0)\}$ and the family covers no neighbourhood of either point.

    What fails is then not the arithmetic but the definition. Subordinacy forces
    $\operatorname{supp}(\eta_\pm) \subseteq \operatorname{Im}(\Psi_\pm)$, so both $\eta_+$ and
    $\eta_-$ vanish at $(1,0)$ and at $(-1,0)$; their sum vanishes there too and cannot be the
    constant $1$ on $S^1$. No partition of unity subordinate to this family exists, and
    \cref{def:integral_scalar_submanifold} never gets started.

    It is worth noticing that the recipe nevertheless returns the right number. Taking
    $\eta_\pm$ summing to $1$ off those two points and substituting $u = \cos t$, which turns
    $(1-u^2)^{-1/2}\,\lvert du\rvert$ into $dt$, gives $\pi + \pi = 2\pi$ again. The reason is the
    one from part \textbf{(a)}: the uncovered set is $\vol_1$-null. So this atlas is illegitimate
    and harmless at once, and only the first of those is a statement about $S^1$ --- the second
    is an accident of the set that was missed.
\end{enumerate}
\end{exercisesolution}
