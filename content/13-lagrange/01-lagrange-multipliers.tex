...
So, $\Hess_xL(x_0,\lambda)$ being positive definite on $T_{x_0}$ is exactly $\Hess h(0)$ being
positive definite, and the unconstrained Hessian test (\cref{thm:hessian_test}, applied to $h$
on the open set $V \subseteq \mathbb{R}^{n-k}$) gives a strict local minimum of $h$ at $0$.
Since $\varphi$ is a local homeomorphism from $V$ onto a neighbourhood of $x_0$ in
$g^{-1}\{0\}$, that is exactly a strict local minimum of $f|_{g^{-1}\{0\}}$ at $x_0$. This proves
\textbf{(1)}; \textbf{(2)} is identical with every inequality reversed. For \textbf{(3)}, if the
quadratic form takes both signs on $T_{x_0}$, then $\Hess h(0)$ is indefinite, so $h$ increases
along one direction through $0 \in V$ and decreases along another (\cref{thm:hessian_test}
again), and transporting those two directions through $\varphi$ shows $f|_{g^{-1}\{0\}}$ does
the same near $x_0$.
\end{proof}
