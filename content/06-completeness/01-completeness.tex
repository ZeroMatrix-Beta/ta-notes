...
\begin{ainote}
The official course script (\S9.1.4, \qt{The reals as the completion of $\mathbb{Q}$}) develops
the converse construction: given \emph{any} incomplete metric space $(X,d)$, one builds a
complete space $(\overline X,\overline d)$ containing an isometric copy of $X$ as a dense subset
--- the \newterm{completion} of $(X,d)$, of which $\mathbb{R}$ as the completion of
$\mathbb{Q}$ is the motivating case (that specific construction of $\mathbb{R}$ itself is marked
\qt{extra material; cf.\ Grundstrukturen} by the script and is not repeated here, since it
belongs to the sibling \emph{Grundstrukturen} project). Corsin's notes never construct a
completion, so \cref{sec:completion_of_a_metric_space} below is a supplement in its own right,
sourced directly from the script rather than from any tutor.
\end{ainote}
...
\begin{remark}
The second bullet of \cref{rem:complete_vs_closed}'s example --- $(0,1) \subset \mathbb{R}$ is
open and not complete --- is not a quirk of that particular interval: script Exercise 9.76 asks
...
fails to be complete, for the same underlying reason. That solution gives the topological proof
via connectedness; an ainote there records the script's more elementary, connectedness-free
alternative.
\end{remark}
