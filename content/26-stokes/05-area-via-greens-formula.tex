...
% 2026-08-13. Three reasons, any one sufficient. Its answer stood directly underneath it, so it
% was never askable; that answer is where \newterm{shoelace formula} and its \germanfor pair are
% introduced, and terminology cannot be introduced inside a solution; and ex:area_ellipse
% immediately below cites the result as an established formula. The solution block also carried no
% [...] title, which is what made it the document's one exercise with no traceable solution.
% It did carry "% Kept inline: solved directly in Corsin Nick/Class Notes/Week 13.pdf, pp. 5--6",
% so the statement-then-answer shape is Corsin's; only the label on it changes.
% Transition: Claude Opus 5 (Extra)
Stokes' theorem turns an integral over a region into one over its boundary. Read in that
direction it computes fluxes and circulations, which is what the previous sections did with it.
Read in the other direction it says something initially surprising: the area of a region is
determined by its boundary curve alone, and can be computed without ever looking inside.

\begin{example}[Area as a boundary functional]
...
\germanfor{Gau\ss sche Trapezformel}
\[A(\gamma) = \frac12\int_0^L \big[\gamma_1(t)\gamma_2'(t) - \gamma_2(t)\gamma_1'(t)\big]\,dt.\]
\end{example}

\begin{remark}[The choice of $F$ is free, and worth exploiting]
% Generator: Claude Opus 5 (Extra)
Any $F$ with $\partial_xF^2 - \partial_yF^1 = 1$ serves, and the three standard choices give
three different-looking formulas for the same number:
\[F = \tfrac12(-y,x) \ \leadsto\ \tfrac12\oint (x\,dy - y\,dx), \qquad
  F = (0,x) \ \leadsto\ \oint x\,dy, \qquad
  F = (-y,0) \ \leadsto\ -\oint y\,dx.\]
The symmetric first choice is the one to quote, but on a particular curve one of the other two is
often much cheaper. If $\gamma_1$ is constant on part of the boundary, $dx$ vanishes there and the
third form kills that piece outright.
\end{remark}
...
\begin{exercise}[Work along paths and Stokes \textnormal{(important)}]
\label{ex:stokes_work_along_paths}
...
\end{enumerate}
\exinfo{This exercise is Problem 12.3 of Problem Sheet 12, Analysis II, Spring Semester 2026.}
\exsol{sol:stokes_work_along_paths}
\end{exercise}
...
\begin{exercise}[True or False (flux on a punctured domain) \textnormal{(important)}]
\label{ex:flux_on_punctured_domain}
...
\end{enumerate}
\exinfo{This exercise is Problem 12.4 of Problem Sheet 12, Analysis II, Spring Semester 2026.}
\exsol{sol:flux_on_punctured_domain}
\end{exercise}
...
\begin{exercise}[Conservative vector fields \textnormal{(important)}]
\label{ex:conservative_vector_field_potential}
...
\exinfo{This exercise is Problem 12.5 of Problem Sheet 12, Analysis II, Spring Semester 2026.}
\exsol{sol:conservative_vector_field_potential}
\end{exercise}
...
\begin{exercise}[Multiple choice (signs in the flux through a rectangle) \textnormal{(important)}]
\label{ex:outward_flux_sign_convention}
...
\begin{enumerate}[label=\textbf{(\alph*)}]
  ...
  \item $-I_b + I_t - I_r + I_l$
\end{enumerate}
\exinfo{This exercise is Problem 12.8 of Problem Sheet 12, Analysis II, Spring Semester 2026.}
\exsol{sol:outward_flux_sign_convention}
\end{exercise}
