...
%
% ⚠️ TO CHECK AGAINST THE SOURCES, not yet done: whether Corsin's Week 6.pdf (pp. 8--9) or
% Analysis_II_Script_v1.pdf carries a proof of the inverse function theorem worth transcribing.
% If one does, transcribe it and cut this note down to a pointer. Do NOT assert in the typeset
% text that "the course does not prove this" until someone has opened those files -- the claim
% below is deliberately only about what is in THIS document, which is what was verified.
% Logged in project-state.md.
...
and is never used again, so it is worth seeing how the two fit together.
