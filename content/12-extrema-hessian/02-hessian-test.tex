...
values on the diagonal. Here it says a single-variable computation --- differentiating
$t\mapsto f(te)$ twice at $t=0$, for every direction $e$ --- already contains everything the
full Hessian matrix does.
\end{remark}
...
(\cref{ex:6.8} is exactly this trap). The official script carries the identical redundant
hypothesis (its Prop.\ 11.15(3): \qt{indefinite and non-degenerate}), so this is not a slip
specific to Corsin's notes --- both sources state the theorem with one clause more than it
needs.
\end{ainote}
...
away from $x_0$ --- a strict local minimum. Negative definite reverses every inequality. And
indefinite means some directions give a positive value and others a negative one, so $f$ rises
along some and falls along others: a saddle. The degenerate-but-not-indefinite case is
inconclusive for a reason now visible --- along an eigenvector with eigenvalue $0$ the quadratic
...
$a,b$}, yet the behaviour at $0$ ranges over every possible case as $a,b$ vary --- $a,b>0$
