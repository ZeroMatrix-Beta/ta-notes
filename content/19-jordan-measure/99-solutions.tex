...
% 8.2 and 8.4 stood in the document unsolved. Written here for the first time.
\section{Solutions}
...
\begin{enumerate}[label=\textbf{\arabic*.}]
  \item \textbf{False.} Take $X := \mathbb{Q}\cap[0,1]$: bounded and countable, yet
    ...
    --- only finitely many intervals are allowed at a time, and finitely many intervals covering
    a dense set must cover the whole of $[0,1]$.
  ...
  \item \textbf{True.} Monotonicity gives $\mu_{out}(D) \leq \mu_{out}([0,1]) = 1$, so only the
    ...
    $\bigcup_{k=1}^N I_k$ is \emph{closed} --- this is where finiteness enters --- and contains
