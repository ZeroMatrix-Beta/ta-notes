...
Not every solution in this chapter is collected here. \Cref{ex:polarisation_formula}
(\cpageref{ex:polarisation_formula}), \cref{ex:cubic_saddle_family}
(\cpageref{ex:cubic_saddle_family}) and \cref{ex:classifying_critical_points_xy}
(\cpageref{ex:classifying_critical_points_xy}) are solved where they appear.

\begin{exercisesolution}[Proof of \cref{ex:ai_extrema_triangle}]
\label{sol:ai_extrema_triangle}
...
\begin{exercisesolution}[Proof of \cref{ex:signature_2x2_matrix}]
\label{sol:signature_2x2_matrix}
...
\begin{exercisesolution}[Solution to \cref{ex:semidefinite_hessian_multiple_choice}]
\label{sol:semidefinite_hessian_multiple_choice}
% Generator: Claude Sonnet 5 (Medium)
...
\begin{exercisesolution}[Solution to \cref{ex:extrema_circle_disk}]
\label{sol:extrema_circle_disk}
% Generator: Claude Sonnet 5 (Medium)
\begin{enumerate}[label=\textbf{(\alph*)}]
  \item With $g(x,y) := x^2+y^2-1$, the Lagrangian is $L(x,y,\lambda) := f(x,y) - \lambda g(x,y)$,
    giving the system
    % Correction: Claude Opus 5 (Ultracode) --- the middle equation read $2y(2-2\lambda)$, twice the
    % true value. With $f = 2x^2+y^2-x$ and $g = x^2+y^2-1$, $\partial_y L = 2y - 2\lambda y
    % = 2y(1-\lambda)$. The error is invisible downstream, since $2-2\lambda$ and $1-\lambda$
    % vanish at the same $\lambda = 1$, so every value below is unaffected; but a reader checking
    % the derivative against $f$ finds it does not match.
    \[0 = \partial_x L = 2x(2-\lambda) - 1, \qquad 0 = \partial_y L = 2y(1-\lambda), \qquad 0 = \partial_\lambda L = -(x^2+y^2-1).\]
...
\begin{exercisesolution}[Solution to \cref{ex:extrema_circle_disk}, continued]
\label{sol:extrema_circle_disk_interior}
% Generator: Claude Sonnet 5 (Medium)
...
\begin{exercisesolution}[Solution to \cref{ex:critical_points_parameterized_gaussian}]
\label{sol:critical_points_parameterized_gaussian}
% Generator: Claude Sonnet 5 (Medium)
...
\label{sol:hessian_parameterized_exp}
