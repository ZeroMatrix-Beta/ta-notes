...
% (0,0), which is why the self-crossing is at the origin and nowhere else.
% Corrected 2026-08-24 (Claude Opus 5, Max): the rest of this block described a superseded
% draft. It gave panel (b)'s circle as centre (-0.45,0) with rightmost point (0.17,0) and a
% stick to (1.15,0), and panel (c)'s marked point as (0.65,0). The drawing uses centre
% (-0.50,0), rightmost point (0.12,0), a stick to (1.25,0), and a marked point at (0.65,-0.65).
% The inline comments on each scope had those right all along, so the duplicate here is dropped
% rather than re-stated: one copy of a number is one copy that can go stale.
\begin{center}
...
% Correction: Claude Opus 5 (Ultracode) --- the example given was the cusp $\{y^2=x^3\}$, which
% this test does not in fact detect. Along its parametrization $t \mapsto (t^2,t^3)$ the field
% $T((t^2,t^3)) := (2,3t)$ is continuous and nowhere zero, and $t$ is a continuous function of the
% point because that parametrization is a homeomorphism onto the set (its inverse is $y/x$ off the
% origin). So a continuous nonvanishing tangent field exists and the test returns nothing; the
% unit tangent does not even jump, tending to $(1,0)$ along both branches. The cusp does fail to
% be a $C^1$ submanifold, but for a reason neither test sees: as a graph over the $y$-axis it is
% $x = y^{2/3}$, which is not $C^1$ at the origin. Replaced by the corner, where the direction
% genuinely does jump and counting genuinely says nothing.
no differentiability. The tangent-field test is the tool when the set stays connected but the
direction of travel jumps. The corner $\{y = \lvert x\rvert\}$ at the origin is the standard
case: removing the origin leaves the expected two pieces, so counting gives nothing away, whereas
a tangent field would have to point along $(1,1)$ as the origin is approached from the right and
along $(1,-1)$ from the left, and no single nonzero vector is both.
\end{remark}
\end{aiexample}
...
% Source: old_exams/fs2023/examsol.pdf (Analysis II, 18 August 2023, Prof. Urs Lang), Aufgabe 2.MC3
% Extractor: Gemini 3.7 Flash (High)
% Classification of four candidate 1-submanifolds of R^2: a punctured algebraic curve (regular value),
% a self-intersecting figure-eight (fails local Euclidean structure), a logarithmic spiral (embedded
% curve despite accumulating on the origin), and the topologist's sine curve with its limiting axis
% (fails local path-connectedness).
\begin{exercise}[Identifying 1-dimensional submanifolds in the plane]
\label{ex:identifying_one_submanifolds_in_plane}
Which of the following subsets of $\mathbb{R}^2$ are $1$-dimensional $C^1$-submanifolds (without boundary) of $\mathbb{R}^2$? Decide each case with proof or counterexample.
\begin{enumerate}[label=\textbf{(\alph*)}]
  \item $M_1 := \big\{(x,y) \in \mathbb{R}^2 \;\big|\; x^4 + y^4 - 2(x^2 - y^2) = 0\big\} \setminus \{(0,0)\}$ (the lemniscate punctured at its self-intersection node).
  \item $M_2 := \big\{(\sin t,\ \sin 2t) \in \mathbb{R}^2 \;\big|\; t \in \mathbb{R}\big\}$ (the figure-eight curve).
  \item $M_3 := \big\{(e^{-t}\cos t,\ e^{-t}\sin t) \in \mathbb{R}^2 \;\big|\; t \in \mathbb{R}\big\}$ (the open logarithmic spiral).
  \item $M_4 := \big\{(x, \sin(1/x)) \in \mathbb{R}^2 \;\big|\; x \in (0, 1)\big\} \cup (\{0\} \times [-1, 1])$ (the topologist's sine curve with its limiting vertical segment).
\end{enumerate}
\exinfo{This exercise is taken from the examination of 18 August 2023 (Prof.\ Urs Lang), Problem 2.MC3.}
\exsol{sol:identifying_one_submanifolds_in_plane}
\end{exercise}
