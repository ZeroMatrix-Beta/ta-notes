...
\begin{enumerate}[label=\textbf{(\alph*)}]
  \item \textbf{The obvious candidate does not exist.} For a map $\mathbb{R}^n \to \mathbb{R}^n$
    the stretching factor is $\lvert\det\rvert$. Here $D\phi(x)$ is $n \times d$ with $d < n$ ---
    a \emph{rectangular} matrix --- and a rectangular matrix has no determinant. So the
    one-line answer is unavailable, and something has to replace it.
...
--- the arc-length integrand of \cref{sec:length_of_curve}. For $d=2$, $n=3$ one computes
