...
\begin{tikzpicture}[>=Latex, scale=1.05,
  ...
  \begin{scope}[shift={(4.7,0)}]
    ...
    % Correction: Claude Opus 5 (High) --- the dotted radial guide above runs from p through the
    % centre, and the normal arrow lies ALONG it, so any label naming that arrow sits on the
    % guide's continuation: the dotted line printed through the word. Moving it does not help --
    % the free space beside the arrow is exactly where the guide goes -- so it takes the white
    % plate from style.md. `text=' rather than a bare colour, or the fill would be repainted.
    \node[lbl, text=gray!45!black, font=\scriptsize, anchor=north west] at (-0.62,0.40) {normal};
...
\end{enumerate}
\exsol{sol:ai_cylinder_geodesics}
\end{aiexercise}
