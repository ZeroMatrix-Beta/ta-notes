...
% Correction: Claude Opus 5 (Ultracode) --- two repairs to the hypotheses. (1) The theorem asked
% for omega to have "compact support in R^n", which does not make supp(omega) ∩ M compact and
% leaves the statement false; see the ainote below for the counterexample. (2) It said
% "orientable", then never fixed an orientation and never said dM carries the induced one, so the
% sign of the right-hand side was undetermined. thm:stokes_classical, which is the case m=2, n=3
% of this theorem, already stated both correctly, so the general statement was strictly weaker
% than its own special case. Nothing downstream changes: every application in the document is to
% a compact M, where both hypotheses are automatic.
\begin{theorem}[Stokes' theorem]
\label{thm:stokes_general}
Let $M \subseteq \mathbb{R}^n$ be an oriented $m$-manifold with (possibly empty) boundary, with
$\partial M$ carrying the induced orientation of \cref{def:induced_orientation}, and let
$\omega \in \Omega^{m-1}(\mathbb{R}^n)$ be a $C^1$ form for which
$\supp\omega \cap M$ is compact. Then
\[\int_{\partial M} \omega = \int_M d\omega.\]
\end{theorem}

\begin{ainote}
\label{note:stokes_hypotheses_are_load_bearing}
Both hypotheses earn their place, and it is worth seeing what each one stops.

\textbf{Compactness of $\supp\omega \cap M$, not of $\supp\omega$.} Requiring only that $\omega$
be compactly supported in $\mathbb{R}^n$ is not enough, because $M$ need not be closed in
$\mathbb{R}^n$ and the support can then run off the end of $M$ without ever leaving a compact set
of $\mathbb{R}^n$. Take $n = m = 1$ and $M = (0,1)$, which has $\partial M = \emptyset$ under
\cref{def:submanifold_with_boundary}, since every one of its points has an open chart. Let
$\omega = f \in \Omega^0(\mathbb{R})$ be compactly supported with $f \equiv 0$ near $0$ and
$f \equiv 1$ near $1$. Then $\int_M d\omega = \int_0^1 f' = 1$, while $\int_{\partial M}\omega$
is an integral over the empty set and vanishes. What fails is that $\supp\omega \cap M = (0,1)$
is not compact. When $M$ itself is compact, which is the case in every application below,
the condition is automatic.

\textbf{An orientation, not merely orientability.} Both integrals change sign when an orientation
is reversed, so a theorem that only asserts one \emph{exists} cannot pin down the sign of either
side. The two orientations must moreover be matched, which is what
\cref{def:induced_orientation} does: it is the outward-vector-first convention, and it is the
same one that produces $\tau = (-\nu_2,\nu_1)$ in Green's theorem and the right-hand rule in
\cref{rem:right_hand_rule_as_cross_product}.
\end{ainote}

\begin{ainote}
\Cref{thm:stokes_classical} was stated ahead of Corsin's own notes, as background
needed to solve \cref{ex:stokes_work_along_paths}, in its classical $\mathbb{R}^3$ curl-theorem form. It is
...
% Added 2026-08-13 (Claude Opus 5, Extra): the line above says the derivation "recovers
% thm:stokes_classical", and the theorem below then states the same result again under its own
% number. A reader meets classical Stokes as two separately numbered theorems with no word
% connecting them. Flagged rather than merged: thm:stokes_classical sits in ch. 25 and is cited
% from ch. 23's solutions, so which of the two should survive is a question about where the
% classical form belongs in the document, not a local edit.
% Transition: Claude Opus 5 (Extra)
Restated in full, and with the orientation convention spelled out, this is the following. It is
\cref{thm:stokes_classical} again, recorded here in the notation of this section because the
convention at the end of the statement is what the rest of the section computes with.
...
\begin{example}[Stokes' theorem on a hemisphere: a linear field]
...
\textbf{Via Gauss.} Since $\divg(\curl F)\equiv0$ (\cref{prop:curl_grad_div_curl_zero}), the flux
...
(\cref{ex:exterior_derivative_gradient_curl}, item \textbf{(c)}) together with the identification
of
...
exactly the content of $d^2=0$ (\cref{thm:exterior_derivative}\textbf{(c)}): a boundary has no
...
\end{enumerate}
For the remaining parts, take $\Omega = B_1$ to be the open unit ball of $\mathbb{R}^2$ and assume
that $\Phi(x) = x$ for every $x \in \partial B_1$.
\begin{enumerate}[label=\textbf{(\alph*)}]
  \setcounter{enumi}{2}
  \item Show that $\displaystyle\int_{B_1} \lvert\det\Jac\Phi\rvert \geq \pi$.
  \item Show that $\Phi(\overline{B_1})$ cannot be contained in the boundary $\partial B_1$.
  \item Show that $0 \in \Phi(\overline{B_1})$.
  \item \textbf{(Bonus.)} Show that $\overline{B_1} \subseteq \Phi(\overline{B_1})$.
\end{enumerate}
Each part may be used in the later ones, even without proof.
\exhint[Official hints]{\textbf{(b):} use Green's formula (i.e.\ 2D Stokes) on the plane.
\textbf{(e):} consider $\widetilde\Phi(x) := \Phi(x)/\lvert\Phi(x)\rvert$.}
\exinfo{This exercise is the whole of Problem 3 of the exam of 21 August 2024 (Prof.\ Joaquim Serra), worth 15 points plus
6 bonus points for the last part.}
\exsol{sol:curl_identity_greens_formula}
\end{exercise}

% Source: exercises_2024/mock.pdf (Analysis II mock examination, Spring Semester 2024, Prof. Joaquim Serra), Exercise 25, part 2, p. 5
% Extractor: Gemini 3.7 Flash (High)
% Constructing a vector potential for a divergence-free field in R^3: while gradient fields
% (0-forms into 1-forms) are standard, finding A with curl A = B (1-forms into 2-forms) by an
% explicit integration ansatz is the second instance of the Poincare lemma.
\begin{exercise}[Constructing a vector potential for a divergence-free field]
\label{ex:constructing_vector_potential_divergence_free}
Let $B \in C^1(\mathbb{R}^3, \mathbb{R}^3)$ be a continuously differentiable vector field on
$\mathbb{R}^3$ satisfying $\divg B = 0$. A vector field $A \in C^1(\mathbb{R}^3, \mathbb{R}^3)$ is called a
\newterm{vector potential} for $B$ if $\curl A = B$.
\begin{enumerate}[label=\textbf{(\alph*)}]
  \item Show that if $A$ is a vector potential for $B$, then for any scalar function $\psi \in C^2(\mathbb{R}^3)$,
    the gauge-transformed field $\widetilde{A} := A + \nabla \psi$ is also a vector potential for $B$.
  \item To construct a vector potential, make the ansatz $A(x,y,z) = (A_1(x,y,z), A_2(x,y,z), 0)\transp$.
    Show that $\curl A = B$ is equivalent to the system of differential equations
    \[
    -\partial_z A_2 = B_1, \qquad \partial_z A_1 = B_2, \qquad \partial_x A_2 - \partial_y A_1 = B_3.
    \]
    Prove that this system always admits an explicit global solution on $\mathbb{R}^3$ given by
    \[
    A_1(x,y,z) := \int_0^z B_2(x,y,s)\,ds, \qquad
    A_2(x,y,z) := -\int_0^z B_1(x,y,s)\,ds + \int_0^x B_3(t,y,0)\,dt.
    \]
  \item Apply this construction to obtain an explicit vector potential $A$ for the divergence-free field
    \[
    B(x,y,z) := (2xy,\ -y^2 + 2yz,\ -z^2)\transp.
    \]
    Verify directly that $\divg B = 0$ and that $\curl A = B$.
\end{enumerate}
\exinfo{This exercise is adapted from the mock examination for Analysis II, Spring Semester 2024 (Prof.\ Joaquim Serra), Exercise 25, part 2.}
\exsol{sol:constructing_vector_potential_divergence_free}
\end{exercise}
