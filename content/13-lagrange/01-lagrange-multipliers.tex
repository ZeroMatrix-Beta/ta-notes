...
$D^2\varphi(0)$ is --- the same cancellation that already made $Dh(0)=0$. So, writing
...
which, since $\varphi$ is a local homeomorphism from $V$ onto a neighbourhood of $x_0$ in
$g^{-1}\{0\}$, is exactly a strict local minimum of $f|_{g^{-1}\{0\}}$ at $x_0$. This proves
...
Hessian test's ordinary $\mathbb{R}^n$ replaced by the tangent space $T_{x_0}$ --- which is
also why $L$, not $f$, is the right function to differentiate: only $L$ has a genuine critical
point at $x_0$ (in the full, unrestricted sense), since $f$ itself typically does not. In
practice \textbf{(1)}--\textbf{(3)} are checked by picking any basis $v_1,\dots,v_{n-k}$ of
$T_{x_0}$ (e.g.\ a basis of $\ker Dg(x_0)$) and testing the definiteness of the
$(n-k)\times(n-k)$ matrix $\big(v_i\transp\Hess_xL(x_0,\lambda)v_j\big)_{i,j}$ with
\cref{thm:hessian_test}, or with the determinant/trace recipe from
\cref{sec:hessian_test} --- there is no need to construct $\varphi$ explicitly, which was only
a proof device.
\end{remark}
...
$\mathbb{R}^2$ --- the unconstrained Hessian test would say nothing. But restricted to the
...
of $f$ over the compact domain is therefore $4$, attained exactly when $y=-x$ --- i.e.\ exactly
at antipodal pairs, where $\|x-y\| = 2$.
\end{exercisesolution}
